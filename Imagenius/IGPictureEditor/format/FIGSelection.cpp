...
int writeToDataObject(LPDATAOBJECT lpDataObject, UINT fid, std::istream& is, HWND hwnd, int bufsize)
{
	...
	if ((bufsize) && (bufsize == sizeof (HANDLE) + 1)) 
    {
		...
		hDib = hBitmapTemp;
		//hDib = (HBITMAP)::OleDuplicateData (hBitmapTemp, Format::F_IGFRAME_DIB, 0);
    }
...
FIGSelection::FIGSelection()
: Format (Format::F_IGSELECTION)
{
}
...
int FIGSelection::write(LPDATAOBJECT pIDataObject, std::istream& is, HWND hwnd, int bufsize)
{  
	return writeToDataObject(pIDataObject, sid_, is, hwnd, bufsize);
}

int FIGSelection::read(LPDATAOBJECT pIDataObject, std::ostream& os, int* size, HWND hwnd)
{
