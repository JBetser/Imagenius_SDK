...
void GetTextandAddLine(wchar_t Line[], HWND hParent, int IDC)
{
    ...
    case 1045:
        switch (lParam)
        {
                ...
                case FD_READ: //Incoming data to receive
                    ...
                    recv (s, (char*)buffer, sizeof(buffer)-1, 0); //Get the text
                ...
                case FD_ACCEPT: //Connection request
                {
                    ...
                    wsprintfA(szAcceptAddr, "Connection from [%s] accepted.",
                      inet_ntoa(from.sin_addr)); 
                      
                    SendMessageA(hStatus, SB_SETTEXT, 0, 
                      (LPARAM)szAcceptAddr);
                }
                break;
        }
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam))
        {
                ...
                case ID_BTN_CLEAR: //Clear edit and disconnect
                {
                    ...
                    {
                        int a = MessageBox(hdwnd, L"Are you sure you want to end the current connection?",
                                        L"End Connection", MB_ICONQUESTION | MB_YESNO);
                        if (a == IDYES)
                        {
                ...
                case ID_BTN_SEND: //Send data
                {
          	        ...
          	        if (len) //If there's text in the reply box...
			        {
			            ...
			            {
			                wchar_t* Data;
			                Data = (wchar_t*)GlobalAlloc(GPTR, (len + 1)*sizeof(wchar_t));
			                
			                GetDlgItemText(hdwnd, ID_EDIT_SEND, Data, len + 1);

			                //Add a carriage return and line feed before sending data
			                wchar_t *szTemp = new wchar_t[len+100];
			                
			                wsprintf(szTemp, L"%s\r\n", Data);
			                
			                send(s, (char*)szTemp, (wcslen(szTemp) + 1)*sizeof(wchar_t), 0); //Send the string
			                
			                SetDlgItemText(hdwnd, ID_EDIT_SEND, L""); //Reset the textbox
			                
			                wsprintf(szTemp, L"[You said:] %s\r\n", Data);
			            ...
			            {
			            //We aren't connected!!
			            MessageBox(hwnd, L"No established connection detected.",
        ...
        switch(LOWORD(wParam))
        {
                case ID_BTN_GO:
                {
                    ...
                    if (len)
                    {
                            ...
                            GetDlgItemTextA(hdwnd, ID_EDIT_HOST, Data, len + 1); //Get text into buffer
                            ...
                            //Couldn't get hostname; assume it's an IP Address
                                ...
                                {
                                    MessageBox(hdwnd, L"Could not connect to remote host.", Title, MB_ICONERROR | MB_OK);
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam))
        {
                case ID_BTN_GO:
                {
                    ...
                    {
                        if (s) closesocket(s);
                        MessageBox(hdwnd, L"Error listening on specified port.", Title, MB_ICONERROR | MB_OK);
                    }                                                            
                    EndDialog(hdwnd, IDOK);
                }
                break;
...
int ListenOnPort(int PortNo)
{
    ...
    wchar_t szTemp[100];
    wsprintf(szTemp, L"Listening on port %d...", PortNo);
