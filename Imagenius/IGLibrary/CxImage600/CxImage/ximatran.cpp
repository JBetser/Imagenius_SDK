...
bool CxImage::GrayScaleRed()
{
	...
	if (head.biBitCount==8 && !head.biClrUsed)
		return true;
	...
	for(long y=0; y < head.biHeight; y++) {
		for(long x=0,x8=0; x < l; x+=3,x8++) {
			img[x8+y*l8]=(BYTE)max (0, *(iSrc+x+2) - (max (*(iSrc+x+1), *(iSrc+x))));
		}
...
////////////////////////////////////////////////////////////////////////////////
bool CxImage::GrayScale(bool bTo8bits)
{
	...
	if (head.biBitCount<=8){
	...
	} else if (bTo8bits) { // convert to 8 bit format
		BYTE *iSrc=info.pImage;
		CxImage ima;
		ima.CopyInfo(*this);
		if (!ima.Create(head.biWidth,head.biHeight,8,info.dwType)) return false;
		...
		for(long y=0; y < head.biHeight; y++) {
			img=ima.GetBits(y);
			for(long x=0; x < l; x+=3) {
				*img++ = (BYTE)RGB2GRAY(*(iSrc+x+2),*(iSrc+x+1),*(iSrc+x+0));
			}
			iSrc+=info.dwEffWidth;
		}
		Transfer(ima);
	}
	else{ // keep 24 bit format
		BYTE *iSrc=info.pImage;
		CxImage ima;
		ima.CopyInfo(*this);
		if (!ima.Create(head.biWidth,head.biHeight,24,info.dwType)) return false;
#if CXIMAGE_SUPPORT_SELECTION
		ima.SelectionCopy(*this);
#endif //CXIMAGE_SUPPORT_SELECTION
#if CXIMAGE_SUPPORT_ALPHA
		...
		for(long y=0; y < head.biHeight; y++) {
			img=ima.GetBits(y);
			for(long x=0; x < l; x+=3) {
				BYTE grayLevel = (BYTE)RGB2GRAY(*(iSrc+x+2),*(iSrc+x+1),*(iSrc+x+0));
				*img++=grayLevel;
				*img++=grayLevel;
				*img++=grayLevel;
			}
...
#if CXIMAGE_SUPPORT_SELECTION
		if (SelectionIsValid()) {
			imgDest.info.rSelectionBox.left = newWidth - 1 - info.rSelectionBox.top;
			imgDest.info.rSelectionBox.right = newWidth - 1 - info.rSelectionBox.bottom;
...
bool CxImage::RotateRight(CxImage* iDst)
{
	...
	if (head.biBitCount == 1) {
		...
		if (SelectionIsValid()){
			...
			imgDest.info.rSelectionBox.bottom = newHeight - 1 - info.rSelectionBox.right;
			imgDest.info.rSelectionBox.top = newHeight - 1 - info.rSelectionBox.left;
			for (y = 0; y < newHeight; y++){
				y2=newHeight-y-1;
				for (x = 0; x < newWidth; x++){
...
////////////////////////////////////////////////////////////////////////////////
bool CxImage::Rotate (int angle)
{
	float fXTr = cosf((float)angle * PI / 180.0f);
	float fYTr = sinf((float)angle * PI / 180.0f);
	return RotateAndResize (fXTr, fYTr, 1.0f);
}
...
bool CxImage::DecreaseBpp(DWORD nbit, bool errordiffusion, RGBQUAD* ppal, DWORD clrimportant)
{
	...
	if (head.biBitCount <  nbit){
		strcpy(info.tcLastError,"DecreaseBpp: target BPP greater than source BPP");
		return false;
	}
	if (head.biBitCount == nbit){
...
bool CxImage::Dither(long method)
{
	...
	CxImage tmpGray;
	tmpGray.Copy(*this);
	tmpGray.GrayScale(true);
	
	CxImage tmp;
	tmp.CopyInfo(tmpGray);
	...
	default:
	{
		...
		for (long y=0;y<head.biHeight;y++){
			...
			for (long x=0;x<head.biWidth;x++){
				...
				level = tmpGray.BlindGetPixelIndex(x,y);
				if (level > 128){
					tmp.SetPixelIndex(x,y,1);
					error = level-255;
				} else {
				...
				nlevel = tmpGray.GetPixelIndex(x+1,y) + (error * 7)/16;
				level = (BYTE)min(255,max(0,(int)nlevel));
				tmpGray.SetPixelIndex(x+1,y,level);
				for(int i=-1; i<2; i++){
					switch(i){
					case -1:
						coeff=3; break;
					...
					nlevel = tmpGray.GetPixelIndex(x+i,y+1) + (error * coeff)/16;
					level = (BYTE)min(255,max(0,(int)nlevel));
					tmpGray.SetPixelIndex(x+i,y+1,level);
				}
			}
		}
	}
	...
	RGBQUAD cBlack = {0,0,0,0};
	RGBQUAD cWhite = {255,255,255,0};
	tmp.SetPaletteColor(0,cBlack);
	tmp.SetPaletteColor(1,cWhite);
	...
	for (long y = ymin; y <= ymax; y++) {
		...
		for (long x = xmin; x <= xmax; x++) {
			if (BlindSelectionIsInside(x,y)){
				BYTE grayLevel = tmpGray.BlindGetPixelIndex(x, y);
				RGBQUAD cGray = {grayLevel,grayLevel,grayLevel,0};
				BlindSetPixelColor (x, y, cGray);
			}
		}
	}
...
////////////////////////////////////////////////////////////////////////////////
bool CxImage::RotateAndResize(float xTransform, float yTransform, float fRate, bool bFullSelect, CxImage *pSrc)
{
	if (abs(xTransform) < 0.0001f)
		xTransform = 0.0f;
	if (abs(xTransform - 1.0f) < 0.0001f)
		xTransform = 1.0f;
	if (abs(yTransform) < 0.0001f)
		yTransform = 0.0f;
	if (abs(yTransform - 1.0f) < 0.0001f)
		yTransform = 1.0f;
	...
	if (!bFullSelect)
		bFullSelect = !SelectionIsValid();
	if (bFullSelect){
		rTransformBox.left = 0; rTransformBox.right = head.biWidth - 1;
		rTransformBox.bottom = 0; rTransformBox.top = head.biHeight - 1;
	}
	long xmin,xmax,ymin,ymax,xpivot,ypivot;
	xpivot = (rTransformBox.left + rTransformBox.right) / 2;
	ypivot = (rTransformBox.top + rTransformBox.bottom) / 2;
	POINT ptTopLeft, ptTopRight, ptBottomLeft, ptBottomRight;
	ptTopLeft.x = xpivot + (int)(((float)(xpivot - rTransformBox.left) * xTransform + (float)(rTransformBox.top - ypivot) * yTransform) * fRate);
	ptTopLeft.y = ypivot + (int)((-(float)(xpivot - rTransformBox.left) * yTransform + (float)(rTransformBox.top - ypivot) * xTransform) * fRate);
	ptTopRight.x = xpivot + (int)(((float)(xpivot - rTransformBox.right) * xTransform + (float)(rTransformBox.top - ypivot) * yTransform) * fRate);
	ptTopRight.y = ypivot + (int)((-(float)(xpivot - rTransformBox.right) * yTransform + (float)(rTransformBox.top - ypivot) * xTransform) * fRate);
	ptBottomLeft.x = xpivot + (int)(((float)(xpivot - rTransformBox.left) * xTransform + (float)(rTransformBox.bottom - ypivot) * yTransform) * fRate);
	ptBottomLeft.y = ypivot + (int)((-(float)(xpivot - rTransformBox.left) * yTransform + (float)(rTransformBox.bottom - ypivot) * xTransform) * fRate);
	ptBottomRight.x = xpivot + (int)(((float)(xpivot - rTransformBox.right) * xTransform + (float)(rTransformBox.bottom - ypivot) * yTransform) * fRate);
	ptBottomRight.y = ypivot + (int)((-(float)(xpivot - rTransformBox.right) * yTransform + (float)(rTransformBox.bottom - ypivot) * xTransform) * fRate);
	xmin = max (0, min (min (ptTopLeft.x, ptTopRight.x), min (ptBottomLeft.x, ptBottomRight.x)));
	xmax = min (head.biWidth - 1, max (max (ptTopLeft.x, ptTopRight.x), max (ptBottomLeft.x, ptBottomRight.x)));
	ymax = min (head.biHeight - 1, max (max (ptTopLeft.y, ptTopRight.y), max (ptBottomLeft.y, ptBottomRight.y)));
	ymin = max (0, min (min (ptTopLeft.y, ptTopRight.y), min (ptBottomLeft.y, ptBottomRight.y)));
	...
	for(long y=ymin; y<=ymax; y++){
		...
		if (bFullSelect){
			if (pSelection){
				if (pAlpha){
					for(long x=xmin; x<=xmax; x++){
						float fsrcX = (float)xpivot + (((float)(x - xpivot) * xTransform - (float)(y - ypivot) * yTransform) / fRate);
						float fsrcY = (float)ypivot + (((float)(x - xpivot) * yTransform + (float)(y - ypivot) * xTransform) / fRate);
						if ((fsrcX >= 0.0f) && (fsrcX < (float)(head.biWidth)) && (fsrcY >= 0.0f) && (fsrcY < (float)head.biHeight)) {
							tmp.SetPixelColor (x, y, pSrc->GetPixelColorInterpolated (fsrcX, fsrcY, CxImage::IM_BILINEAR, CxImage::OM_BACKGROUND), true);
							tmp.SelectionSet (x, y, *pSrc->SelectionGetPointer ((int)fsrcX, (int)fsrcY));
							tmp.AlphaSet (x, y, pSrc->AlphaGet((int)fsrcX, (int)fsrcY));
						}
					}
				}
				else{
			...
			for(long x=xmin; x<xmax; x++){
				float fsrcX = (float)xpivot + (((float)(x - xpivot) * xTransform - (float)(y - ypivot) * yTransform) / fRate);
				float fsrcY = (float)ypivot + (((float)(x - xpivot) * yTransform + (float)(y - ypivot) * xTransform) / fRate);
				if (pSrc->SelectionIsInside ((int)fsrcX, (int)fsrcY)) {
					tmp.SetPixelColor (x, y, pSrc->GetPixelColorInterpolated (fsrcX, fsrcY, CxImage::IM_BILINEAR, CxImage::OM_BACKGROUND), true);
					tmp.SelectionSet (x, y, *pSrc->SelectionGetPointer ((int)fsrcX, (int)fsrcY));
				}
			}
...
bool CxImage::Expand(long newx, long newy, RGBQUAD canvascolor, CxImage* iDst)
{
    ...
    return Expand(0, 0, newx - head.biWidth, newy - head.biHeight, canvascolor, iDst);
}
////////////////////////////////////////////////////////////////////////////////
/**
...
// smile range: [0;1]
bool CxImage::Smile(float fSmileSizeL, float fSmileSizeR, const RECT& rcReg, const RECT& rcEyeLeft, const RECT& rcEyeRight, const RECT& rcMouth)
{
	if (!pDib || fSmileSizeL < 0.0f || fSmileSizeL > 1.0f || fSmileSizeR < 0.0f || fSmileSizeR > 1.0f) return false;
	...
	bool bMouthValid = (rcMouth.top < rcMouth.bottom) && (rcMouth.left < rcMouth.right) && (rcMouth.top > 0) && (rcMouth.left > 0);

	int xmid1 = bMouthValid ? rcMouth.left : rcReg.left + (long) ((float)(rcReg.right - rcReg.left) / 5.0f);
	int xmid2 = bMouthValid ? rcMouth.right : rcReg.left + (long) ((4.0f * (float)(rcReg.right - rcReg.left)) / 5.0f);
	float fStdDevX = bMouthValid ? (float)(rcMouth.right - rcMouth.left) / 10.0f : (float)(xmid1 - rcReg.left) / 3.0f;
	int nMeanEyesY = ((rcEyeLeft.bottom + rcEyeLeft.top) / 2 + (rcEyeRight.bottom + rcEyeRight.top) / 2) / 2;
	int ymid = bMouthValid ? rcMouth.bottom : max(rcReg.top + (long) ((float)(rcReg.bottom - rcReg.top) / 4.0f), nMeanEyesY - (rcReg.right - rcReg.left) / 2);
	float fStdDevY = (float)(ymid - rcReg.top) / 4.0f;
	float fSmileSizeMaxLR = max(fSmileSizeL, fSmileSizeR);
	float fSmileSizeY = fStdDevY * (1.0f + 2.0f * fSmileSizeMaxLR);
	float fSmileSizeX = fStdDevY * (1.0f + fSmileSizeMaxLR);

	for(long y=rcReg.top; y<rcReg.bottom; y++){
		info.nProgress = (long)(100*(y-rcReg.bottom)/(rcReg.bottom-rcReg.top));
		float fCoeffY = CxMath::NormalDistrib ((float)ymid, fStdDevY, (float)y, false, 2.0f);
		for(long x=rcReg.left; x<rcReg.right; x++){
#if CXIMAGE_SUPPORT_SELECTION
			if (BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				float fCoeffX1 = CxMath::NormalDistrib ((float)xmid1, fStdDevX, (float)x, false);
				float fCoeffX2 = CxMath::NormalDistrib ((float)xmid2, fStdDevX, (float)x, false);
				int ny = (int)((float)y - (fSmileSizeY - powf (fSmileSizeY, 1.0f - 0.5f * fCoeffX1)) * fCoeffY * fSmileSizeL - (fSmileSizeY - powf (fSmileSizeY, 1.0f - 0.5f * fCoeffX2)) * fCoeffY * fSmileSizeR);
				int nx = (int)((float)x + (fSmileSizeX - powf (fSmileSizeX, 1.0f - 0.5f * fCoeffX1)) * fCoeffY * fSmileSizeL - (fSmileSizeX - powf (fSmileSizeX, 1.0f - 0.5f * fCoeffX2)) * fCoeffY * fSmileSizeR);
				if (head.biClrUsed==0){
					tmp.SetPixelColor(x,y,GetPixelColor(nx,ny));
				} else {
					tmp.SetPixelIndex(x,y,GetPixelIndex(nx,ny));
...
bool CxImage::Morphing(int nPosX, int nPosY, float fMorphingDirectionX, float fMorphingDirectionY, float fRadius, int nType, bool bDisableSel)
{
	// check arguments
	if (!pDib || nPosX < 0 || nPosX >= head.biWidth || nPosY < 0 || nPosY >= head.biHeight ||
		(fMorphingDirectionX == 0.0f && fMorphingDirectionY == 0.0f)) 
		return false;
	if (fRadius < 1.0f){
		fRadius = sqrtf(fMorphingDirectionX * fMorphingDirectionX + fMorphingDirectionY * fMorphingDirectionY);
		if (fRadius < 1.0f)
			return false;
		if (nType != CXIMAGE_MORPHING_TYPE_STAR){
			// set the center of transformation in half-way
			nPosX = max (0, min (head.biWidth - 1, nPosX + (int)(fMorphingDirectionX / 2.0f)));
			nPosY = max (0, min (head.biHeight - 1, nPosY + (int)(fMorphingDirectionY / 2.0f)));
		}
	}
	...
	float fStdDev = fRadius / 3.0f;	// below 3 sigma, normal distribution is < 1%
	float fMorphingSize = fRadius * 2.0f;
	float fMorphingMinY = max(0.0f, nPosY - fMorphingSize / 2.0f);
	float fMorphingMinX = max(0.0f, nPosX - fMorphingSize / 2.0f);
	float fMorphingMaxY = min((float)(head.biHeight - 1), fMorphingMinY + fMorphingSize);
	float fMorphingMaxX = min((float)(head.biWidth - 1), fMorphingMinX + fMorphingSize);
	
	switch (nType){
	case CXIMAGE_MORPHING_TYPE_RADIAL:	
		// rotate the unitary vector X and normalize
		float Xu, Yu;
		CxMath::VecRotate(1.0f, 0.0f, fMorphingDirectionX, fMorphingDirectionY, Xu, Yu);	
		// take the opposite direction	
		Xu *= -1;
		Yu *= -1;
		for(long y = (long)fMorphingMinY; y < (long)fMorphingMaxY; y++){
			info.nProgress = (long)(100.0f*(float)(y-fMorphingMinY)/fMorphingSize);
			float fCoeffY = CxMath::NormalDistrib ((float)nPosY, fStdDev, (float)y, false);
			for(long x = (long)fMorphingMinX; x < (long)fMorphingMaxX; x++){
#if CXIMAGE_SUPPORT_SELECTION
				if (bDisableSel || BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					float fCoeffX = CxMath::NormalDistrib ((float)nPosX, fStdDev, (float)x, false);
					int nx = max (0, min (head.biWidth - 1, x + (int)(fCoeffX * fCoeffY * fRadius * Xu)));
					int ny = max (0, min (head.biHeight - 1, y + (int)(fCoeffX * fCoeffY * fRadius * Yu)));
...
#endif //CXIMAGE_SUPPORT_ALPHA
#if CXIMAGE_SUPPORT_SELECTION
					tmp.SelectionSet(x, y, 255);
#endif //CXIMAGE_SUPPORT_SELECTION
				}
			}
		}
		break;
	case CXIMAGE_MORPHING_TYPE_DIRECTIONNAL:
		fStdDev /= 3.0f;
	case CXIMAGE_MORPHING_TYPE_WIDE:
		{
			// rotate the unitary vector Y and normalize
			float Xu, Yu;
			CxMath::VecRotate(0.0f, 1.0f, fMorphingDirectionX, fMorphingDirectionY, Xu, Yu);	
			// take the opposite y direction	
			Yu *= -1;
			// rotate the transformation center
			float Xcr, Ycr;
			CxMath::VecRotate((float)nPosX, (float)nPosY, Xu, Yu, Xcr, Ycr);
			float fMorphingSizeX = fMorphingSize * fMorphingDirectionX;
			float fMorphingSizeY = fMorphingSize * fMorphingDirectionY;
			for(long y = (long)fMorphingMinY; y < (long)fMorphingMaxY; y++){
				info.nProgress = (long)(100.0f*(float)(y-fMorphingMinY)/fMorphingSize);			
				for(long x = (long)fMorphingMinX; x < (long)fMorphingMaxX; x++){
#if CXIMAGE_SUPPORT_SELECTION
					...
					{
						float Xr, Yr;
						CxMath::VecRotate ((float)x, (float)y, Xu, Yu, Xr, Yr);		// Passage tranform to rotated vectorial space
						float fCoeffX = CxMath::NormalDistrib (Xcr, fStdDev, Xr, false); // Normal law morphing on X-rotated coord
						float fDist = abs(Yr - Ycr);  //sqrtf (powf (x - (fMorphingMinX + fMorphingMaxX) / 2.0f, 2.0f) + powf (y - (fMorphingMinY + fMorphingMaxY) / 2.0f, 2.0f));
						if (fDist > fRadius)
							fDist = fRadius;
						float fCoeffY = (fRadius - fDist) / fRadius;
						float fOffset = powf (fRadius, 1.0f - 0.5f * fCoeffX * fCoeffY);	// Exponential morphing on Y-rotated coord
						float Ytr = Yr + ((fRadius - fOffset) * fCoeffY) / 2.0f;
						float Xtr = Xr;
				
						float Xt, Yt;
						CxMath::VecRotate (Xtr, Ytr, Xu, -Yu, Xt, Yt);	// Passage tranform to original vectorial space
						...
						int nx = max (0, min (head.biWidth - 1, (int)Xt));
						int ny = max (0, min (head.biHeight - 1, (int)Yt));			
		...
		break;
		case CXIMAGE_MORPHING_TYPE_STAR:
		{
			// rotate the transformation center
			float Xcr, Ycr;
			CxMath::VecRotate((float)nPosX, (float)nPosY, fMorphingDirectionX, fMorphingDirectionY, Xcr, Ycr);
			float fMorphingSizeX = fMorphingSize * fMorphingDirectionX;
			float fMorphingSizeY = fMorphingSize * fMorphingDirectionY;
			for(long y = (long)fMorphingMinY; y < (long)fMorphingMaxY; y++){
				info.nProgress = (long)(100.0f*(float)(y-fMorphingMinY)/fMorphingSize);			
				for(long x = (long)fMorphingMinX; x < (long)fMorphingMaxX; x++){
					...
					{
						float Xr, Yr;
						CxMath::VecRotate ((float)x, (float)y, fMorphingDirectionX, fMorphingDirectionY, Xr, Yr);					

						float Xtr = Xr;
						float Ytr = Yr + fRadius / (max (1.0f, abs (Ycr - Yr)) * max (1.0f, abs (Xcr - Xr)));

						float Xt, Yt;
						CxMath::VecRotate (Xtr, Ytr, fMorphingDirectionX, -fMorphingDirectionY, Xt, Yt);	
						int nx = max (0, min (head.biWidth - 1, (int)Xt));
						int ny = max (0, min (head.biHeight - 1, (int)Yt));			
...
bool CxImage::QIShrink(long newx, long newy, CxImage* const iDst, bool bChangeBpp)
{
	...
	if (newx>head.biWidth || newy>head.biHeight) { 
		...
		strcpy(info.tcLastError,"QIShrink can't enlarge image");
		return false;
	}
