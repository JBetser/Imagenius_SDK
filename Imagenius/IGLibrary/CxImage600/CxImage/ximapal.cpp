...
////////////////////////////////////////////////////////////////////////////////
void CxImage::DrawLine(int StartX, int EndX, int StartY, int EndY, const CxBrush& brush)
{
	if (!pDib) return;
	//////////////////////////////////////////////////////
	...
	if (deltax >= deltay)         // There is at least one x-value for every y-value
	{
		...
		fDer = (float)deltay / (float)deltax;
	}
	else                          // There is at least one y-value for every x-value
	{
		...
		fDer = (float)deltax / (float)deltay;
	}
	POINT pt; pt.x = x; pt.y = y;
	BYTE *pBits = GetPixelColor24 (pt);
	BYTE *pAlpha = AlphaGetPointer (x, y);
	BYTE *pSel = SelectionGetPointer (x, y);
	if ((info.rSelectionBox.left >= info.rSelectionBox.right) ||
		(info.rSelectionBox.bottom >= info.rSelectionBox.top))
		pSel = NULL;	// no selection
	CxImage cxOriginal (*this, true, false, false);
	BYTE *pOriginalBits = cxOriginal.GetPixelColor24 (pt);
	
	for (int curpixel = 0; curpixel <= numpixels; curpixel++)
	{
		// Draw the current theorical pixel
		//if (bDerByX)
		brush.Apply ((float)x, (float)y /*(x - StartX) * fDer + (float)StartY*/, pOriginalBits, pBits, pAlpha, pSel, head.biWidth, head.biHeight, info.dwEffWidth);
		//else
			//brush.Apply ((float)(y -  StartY) * fDer + (float)StartX, (float)y, pOriginalBits, pBits, pAlpha, pSel, head.biWidth, head.biHeight, info.dwEffWidth);
		...
		if (num >= den)             // Check if numerator >= denominator
		{
			...
			pBits += yinc1*info.dwEffWidth + xinc1*3;
			pOriginalBits += yinc1*info.dwEffWidth + xinc1*3;
			pAlpha += yinc1*head.biWidth + xinc1;
			if (pSel)
				pSel += yinc1*head.biWidth + xinc1;
		}
		x += xinc2;                 // Change the x as appropriate
		y += yinc2;                 // Change the y as appropriate
		pBits += yinc2*info.dwEffWidth + xinc2*3;
		pOriginalBits += yinc2*info.dwEffWidth + xinc2*3;
		pAlpha += yinc2*head.biWidth + xinc2;
		if (pSel)
			pSel += yinc2*head.biWidth + xinc2;
	}
}
