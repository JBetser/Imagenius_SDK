 ...
 */ //</DP>
	...
	ProgressSetRange (cinfo.output_height, 0);
	ProgressSetMessage (L"Decoding JPEG...");	
	...
	*/
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		JDIMENSION nNbLines = jpeg_read_scanlines(&cinfo, buffer, 1);
		...
		if ((cinfo.num_components==4)&&(cinfo.quantize_colors==FALSE)){
			...
			iter.PrevRow();
		::SendMessageW (info.hProgress, UM_PROGRESS_STEPIT, nNbLines, 0);
	}
...
bool CxImageJPG::Encode(CxFile * hFile)
{
	...
	if (head.biClrUsed!=0 && !IsGrayScale()){
		strcpy(info.tcLastError,"JPEG can save only RGB or GreyScale images");
		return false;
	}	
	...
	*/
	...
	if (setjmp(jerr.setjmp_buffer)) {
		...
		* We need to clean up the JPEG object, close the input file, and return.
		*/
		strcpy(info.tcLastError, jerr.buffer); //<CSC>
...
//#endif
	...
	*/
	...
	while (cinfo.next_scanline < cinfo.image_height) {
		...
		iter.PrevRow();
		JDIMENSION nNbLines = jpeg_write_scanlines(&cinfo, buffer, 1);
		::SendMessageW (info.hProgress, UM_PROGRESS_STEPIT, nNbLines, 0);
	}
