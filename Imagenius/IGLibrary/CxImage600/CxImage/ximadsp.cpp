	...
	for (long y=0;y<head.biHeight;y++){
		...
		for (long x=0;x<head.biWidth;x++){
			if (BlindGetPixelIndex(x,y)>level)
				BlindSetPixelIndex(x,y,255);
			else
				BlindSetPixelIndex(x,y,0);
		}
	}
	/*
...
bool CxImage::ThresholdAnd(CxImage* pThresholdMask, BYTE filtLevel)
{
	...
	if (!pThresholdMask->IsValid() ||
		...
		pThresholdMask->GetHeight() != GetHeight()){
			strcpy(info.tcLastError,"invalid ThresholdMask");
			return false;
	}
	...
	for (long y=0;y<head.biHeight;y++){
		...
		if (info.nEscape) break;
		for (long x=0;x<head.biWidth;x++){
			if (pThresholdMask->BlindGetPixelIndex(x,y) == 0)
				BlindSetPixelIndex(x,y,filtLevel);
		}
	}
	return true;
}
 ...
 * \return true if everything is ok
 */
bool CxImage::Colorize(BYTE hue, BYTE sat, BYTE val, float blend, bool bMixVal)
{
			...
			for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (bFullBlend){
						...
						color.rgbGreen=sat;
						if (bMixVal)
							color.rgbBlue = (BYTE)(color.rgbBlue + val) / 2;
...
bool CxImage::ColorizeTurb(BYTE red, BYTE green, BYTE blue, float blend)
{
	if (!pDib) return false;
	...
	colRGB.rgbRed = red;
	colRGB.rgbBlue = blue;
	colRGB.rgbGreen = green;
	colRGB.rgbReserved = 0x00;
	RGBQUAD colHSL = RGBtoHSL (colRGB);	
	BYTE hue = colHSL.rgbRed;
	BYTE sat = colHSL.rgbGreen;
	BYTE val = colHSL.rgbBlue;
	if (head.biClrUsed!=0)
		return false;
	...
	IGRandomGenerator randomGenSmallPattern (32);
	float fRandomSmallStepX = (float)(xmax - xmin + 1) / 32.0f;
	float fRandomSmallStepY = (float)(ymax - ymin + 1) / 32.0f;
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				// value and saturation blending
				RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
				RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);	
				colOrigHSL.rgbRed = hue;			
				colOrigRGB = HSLtoRGB(colOrigHSL);
				
				// RGB blending
				int f0 = (int)(255.0f * blend * (0.5f + (randomGenSmallPattern.smoothNoise (((float)(x - xmin)) / fRandomSmallStepX, ((float)(y - ymin)) / fRandomSmallStepY) / 2.0f)));
				int f1 = 255 - f0;				
				colOrigRGB.rgbRed	= (BYTE)((colRGB.rgbRed * f0 + colOrigRGB.rgbRed * f1)>>8);
				colOrigRGB.rgbGreen	= (BYTE)((colRGB.rgbGreen * f0 + colOrigRGB.rgbGreen * f1)>>8);
				colOrigRGB.rgbBlue	= (BYTE)((colRGB.rgbBlue * f0 + colOrigRGB.rgbBlue * f1)>>8);
				BlindSetPixelColor (x, y, colOrigRGB);
			}
		}
	}
	return true;
...
bool CxImage::FaceColorize(BYTE red, BYTE green, BYTE blue, float blend, RECT rcEyeLeft, RECT rcEyeRight, RECT rcMouth, bool bNoTurbulence, bool bFun)
{
	if (!pDib) return false;
	...
	float fFaceWidth = (float)(xmax - xmin + 1);
	float fFaceHeight = (float)(ymax - ymin + 1);
	float fYoX = fFaceHeight / fFaceWidth;
	IGSTRUCTELEM_ELLIPSE faceEllipse (&fYoX);
	float fEYoX= 0.5f;
	IGSTRUCTELEM_ELLIPSE eyeEllipse (&fEYoX);
	float fMYoX= 0.5f;
	IGSTRUCTELEM_ELLIPSE mouthEllipse (&fMYoX);
	POINT ptFaceCtr,ptEyeLeftCtr,ptEyeRightCtr,ptMouthCtr;
	ptFaceCtr.x = (rcEyeRight.left + rcEyeLeft.right) / 2;
	ptFaceCtr.y = min(rcEyeRight.top,rcEyeLeft.bottom);
	ptEyeLeftCtr.x = (rcEyeLeft.left + rcEyeLeft.right) / 2;
	ptEyeLeftCtr.y = (rcEyeLeft.top + rcEyeLeft.bottom) / 2;
	ptEyeRightCtr.x = (rcEyeRight.left + rcEyeRight.right) / 2;
	ptEyeRightCtr.y = (rcEyeRight.top + rcEyeRight.bottom) / 2;	
	ptMouthCtr.x = (rcMouth.left + rcMouth.right) / 2;
	ptMouthCtr.y = (rcMouth.top + rcMouth.bottom) / 2;	
	long nLongRayLeft = abs (xmin - ptFaceCtr.x);
	long nLongRayRight = abs (xmax - ptFaceCtr.x);
	long nLongRayTop = abs (ymin - ptFaceCtr.y);
	long nLongRayBottom = abs (ymax - ptFaceCtr.y);
	long nShortRay = min ((rcEyeRight.right - rcEyeLeft.left) / 2, min (nLongRayLeft - 20, nLongRayRight - 20));
	long nEyeRay = (long)(0.3f * (float)(rcEyeRight.bottom - rcEyeRight.top));
	long nMouthRay = (long)(0.5f * (float)(rcMouth.bottom - rcMouth.top));
	
	IGRandomGenerator randomGenBigPattern (8);
	IGRandomGenerator randomGenSmallPattern (32);
	float fRandomBigStepX = fFaceWidth / 8.0f;
	float fRandomBigStepY = fFaceHeight / 8.0f;
	float fRandomSmallStepX = fFaceWidth / 32.0f;
	float fRandomSmallStepY = fFaceHeight / 32.0f;
	...
	for(long y=ymin; y<=ymax; y++){
		...
		for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
				RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);				
				float evlEyeL = min (1.0f, eyeEllipse(x - ptEyeLeftCtr.x, y - ptEyeLeftCtr.y) / (float)nEyeRay);
				float evlEyeR = min (1.0f, eyeEllipse(x - ptEyeRightCtr.x, y - ptEyeRightCtr.y) / (float)nEyeRay);
				float evlMouth = min (1.0f, mouthEllipse(x - ptMouthCtr.x, y - ptMouthCtr.y) / (float)nMouthRay);
				float dxL = 1.0f / (float)(nLongRayLeft - nShortRay);
				float dxR = - 1.0f / (float)(nLongRayRight - nShortRay);
				float exL = - dxL * (ptFaceCtr.x - (float)nLongRayLeft);
				float exR = - dxR * (ptFaceCtr.x + (float)nLongRayRight);
				float dyL = 1.0f / (float)(nLongRayTop - nShortRay);
				float dyR = - 1.0f / (float)(nLongRayBottom - nShortRay);
				float eyL = - dyL * (ptFaceCtr.y - (float)nLongRayTop);
				float eyR = - dyR * (ptFaceCtr.y + (float)nLongRayBottom);
				float faceCoeff = min (1.0f, max (0.0f, dxL * x + exL)) * min (1.0f, max (0.0f, dxR * x + exR)) * 
									min (1.0f, max (0.0f, dyL * y + eyL)) * min (1.0f, max (0.0f, dyR * y + eyR));

				if (bFun) {
					float f0 = min(1.0f, evlEyeL * evlEyeR);
					float f1 = 1.0f - f0;
					colOrigHSL.rgbRed=(BYTE)(hue * f0 + colOrigHSL.rgbRed * f1);
					colOrigHSL.rgbGreen=(BYTE)(sat * f0 + colOrigHSL.rgbGreen * f1);
					if (f0 > 0.99f){
						if (bNoTurbulence){
							if (evlMouth * faceCoeff > 0.0f)
								colOrigHSL.rgbRed = hue;
						}
						else{
							if (evlMouth * faceCoeff > 1.0f)
								colOrigHSL.rgbRed = hue;			
						}
					}
				}
					...
					if (bNoTurbulence){
						if (evlEyeL * evlEyeR * evlMouth * faceCoeff > 0.0f)
							colOrigHSL.rgbRed = hue;
					}
					else{
						if (evlEyeL * evlEyeR * evlMouth * faceCoeff > 1.0f)
							colOrigHSL.rgbRed = hue;			
					}
				}
				colOrigRGB = HSLtoRGB(colOrigHSL);
				...
				// RGB blending
				float distToCtr = faceEllipse (x - ptFaceCtr.x, y - ptFaceCtr.y);
				float fCurblend = blend * evlEyeL * evlEyeR * evlMouth * faceCoeff;
				int f0 = (int)(255.0f * fCurblend * (bNoTurbulence ? 1.0f : (0.5f + (randomGenSmallPattern.smoothNoise (((float)(x - xmin)) / fRandomSmallStepX, ((float)(y - ymin)) / fRandomSmallStepY) / 2.0f))));
				int f1 = 255 - f0;				
				colOrigRGB.rgbRed	= (BYTE)((colRGB.rgbRed * f0 + colOrigRGB.rgbRed * f1)>>8);
				colOrigRGB.rgbGreen	= (BYTE)((colRGB.rgbGreen * f0 + colOrigRGB.rgbGreen * f1)>>8);
				colOrigRGB.rgbBlue	= (BYTE)((colRGB.rgbBlue * f0 + colOrigRGB.rgbBlue * f1)>>8);
...
bool CxImage::SetEyeHueSat (BYTE hue, BYTE sat, RECT rcEye, BYTE valThreshold, bool bMix, bool bUseSelect)
{
	if (!pDib) return false;
	...
	int nWidth = rcEye.right - rcEye.left;
	int nHeight = rcEye.bottom - rcEye.top;
	long xmin,xmax,ymin,ymax;
	xmin = max (0L, rcEye.left - 2 * nWidth); xmax = min (head.biWidth - 1, rcEye.right + 2 * nWidth);
	ymin = max (0L, rcEye.top - 2 * nHeight); ymax = min (head.biHeight - 1, rcEye.bottom + 2 * nHeight);

	POINT ptCtr;
	ptCtr.x = (rcEye.left + rcEye.right) / 2;
	ptCtr.y = (rcEye.top + rcEye.bottom) / 2;
	long nEyeRay = (long)(0.1f * (float)(rcEye.bottom - rcEye.top));
	long nEyeLongRay = 3 * nEyeRay;
	float fEYoX = 0.5f;
	IGSTRUCTELEM_ELLIPSE eyeEllipse (&fEYoX);
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (eyeEllipse(x - ptCtr.x, y - ptCtr.y, nEyeLongRay))
					{
						// value and saturation blending
						RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
						RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);	
						float distCtr = eyeEllipse(x - ptCtr.x, y - ptCtr.y);
						float evlEye = min (1.0f, max (0.0f, (float)(nEyeLongRay - distCtr) / (float)(nEyeLongRay - nEyeRay)));
						colOrigHSL.rgbRed = hue;	
						if (colOrigHSL.rgbBlue > valThreshold) {
							float fPower = ((float)colOrigHSL.rgbBlue - (float)valThreshold) / (255.0f - (float)valThreshold);
							colOrigHSL.rgbGreen = (BYTE)max (0.0f, min (255.0f, (float)colOrigRGB.rgbGreen + evlEye * fPower * (float)sat));
						}
						RGBQUAD colRGB = HSLtoRGB(colOrigHSL);

						// RGB blending
						if (bMix){
							colRGB.rgbRed = (BYTE)max (0.0f, min (255.0f, 0.5f * (float)colOrigRGB.rgbRed + 0.5f * (float)colRGB.rgbRed));
							colRGB.rgbGreen = (BYTE)max (0.0f, min (255.0f, 0.5f * (float)colOrigRGB.rgbGreen + 0.5f * (float)colRGB.rgbGreen));
							colRGB.rgbBlue = (BYTE)max (0.0f, min (255.0f, 0.5f * (float)colOrigRGB.rgbBlue + 0.5f * (float)colRGB.rgbBlue));
						}
					}
				}
			}
...
bool CxImage::AddEyeRGB (RGBQUAD rgbColRGB, RECT rcEye, bool bUseSelect)
{
	if (!pDib) return false;
	...
	ptCtr.x = (rcEye.left + rcEye.right) / 2;
	ptCtr.y = (rcEye.top + rcEye.bottom) / 2;
	long nEyeRay = (long)(0.3f * (float)(rcEye.bottom - rcEye.top));
	long nEyeLongRay = 3 * nEyeRay;
	float fEYoX = 0.5f;
	IGSTRUCTELEM_ELLIPSE eyeEllipse (&fEYoX);
			...
			for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (eyeEllipse(x - ptCtr.x, y - ptCtr.y, nEyeLongRay))
					{
						...
						float evlEye = min (1.0f, max (0.0f, (float)(nEyeLongRay - distCtr) / (float)(nEyeLongRay - nEyeRay)));
						RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
						colOrigRGB.rgbRed = (BYTE)max (0.0f, min (255.0f, (float)colOrigRGB.rgbRed + evlEye * (float)rgbColRGB.rgbRed));
						colOrigRGB.rgbGreen = (BYTE)max (0.0f, min (255.0f, (float)colOrigRGB.rgbGreen + evlEye * (float)rgbColRGB.rgbGreen));
						colOrigRGB.rgbBlue = (BYTE)max (0.0f, min (255.0f, (float)colOrigRGB.rgbBlue + evlEye * (float)rgbColRGB.rgbBlue));
						BlindSetPixelColor (x, y, colOrigRGB);
					}
				}
			}
...
bool CxImage::AddEyeSaturation (short sat, RECT rcEye, bool bUseSelect)
{
	if (!pDib) return false;
			...
			for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (eyeEllipse(x - ptCtr.x, y - ptCtr.y, nEyeLongRay))
					{
						...
						RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
						RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);	
						colOrigHSL.rgbGreen = (BYTE)max (0.0f, min (255.0f, (float)colOrigHSL.rgbGreen + evlEye * (float)sat));
						colOrigRGB = HSLtoRGB(colOrigHSL);
						BlindSetPixelColor (x, y, colOrigRGB);
					}
				}
...
bool CxImage::AddEyeValue (short val, RECT rcEye, bool bUseSelect)
{
	if (!pDib) return false;
			...
			for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (eyeEllipse(x - ptCtr.x, y - ptCtr.y, nEyeLongRay))
					{
						...
						RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
						RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);	
						colOrigHSL.rgbBlue = (BYTE)max (0.0f, min (255.0f, (float)colOrigHSL.rgbBlue + evlEye * (float)val));				
						colOrigRGB = HSLtoRGB(colOrigHSL);
						BlindSetPixelColor (x, y, colOrigRGB);
					}
				}
...
bool CxImage::ScaleEyeValue (float fScale, RECT rcEye, bool bUseSelect)
{
	if (!pDib) return false;
			...
			for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					if (eyeEllipse(x - ptCtr.x, y - ptCtr.y, nEyeLongRay))
					{
						...
						RGBQUAD colOrigRGB = BlindGetPixelColor(x,y);
						RGBQUAD colOrigHSL = RGBtoHSL (colOrigRGB);	
						colOrigHSL.rgbBlue = (BYTE)max (0.0f, min (255.0f, (float)colOrigHSL.rgbBlue + evlEye * fScale * (float)colOrigHSL.rgbBlue));				
						colOrigRGB = HSLtoRGB(colOrigHSL);
						BlindSetPixelColor (x, y, colOrigRGB);
					}
				}
...
bool CxImage::kmeanClustering()
{
  ...
  IGSmartLayer tmpLayer;
	...
	size = tmpLayer.head.biWidth * tmpLayer.head.biHeight;
  ...
  for(long y=ymin, i=0; y<=ymax; y++){
		BYTE *pSrc = tmpLayer.GetBits(y);
		for(long x=xmin; x<=xmax; x++, i++){
  ...
  for (i = 0; i < size; i++) {
    ...
    if (idx == 0)
	{
		color->data.fl[idx * 3 + 0] = 255;
		color->data.fl[idx * 3 + 1] = 255;
		color->data.fl[idx * 3 + 2] = 255;
	}
	else
	{
		color->data.fl[idx * 3 + 0] = 0;
		color->data.fl[idx * 3 + 1] = 0;
		color->data.fl[idx * 3 + 2] = 0;
	}
  }
	...
	for(long y=ymin; y<=ymax; y++){
		...
		BYTE *pDst = GetBits(y);
		for(long x=xmin; x<=xmax; x++){
				int idx = clusters->data.i[y * head.biWidth + x];
				*pDst++ = (char) color->data.fl[idx * 3 + 0];
				*pDst++ = (char) color->data.fl[idx * 3 + 1];
				*pDst++ = (char) color->data.fl[idx * 3 + 2];
			}
	}
			...
			for(long x=xmin; x<=xmax; x++, iY1++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					...
					if (Kfactor==0 || ksumcur==0){
						cPtr2[iY1] = (BYTE)min(255, max(0,(int)(abs(b) + Koffset)));
					} else if (ksumtot == ksumcur) {
						cPtr2[iY1] = (BYTE)min(255, max(0,(int)(abs(b)/Kfactor + Koffset)));
					} else {
						cPtr2[iY1] = (BYTE)min(255, max(0,(int)((abs(b)*ksumtot)/(ksumcur*Kfactor) + Koffset)));
					}
				}
			}
		}
		...
		for(long y=ymin; y<=ymax; y++){
			...
			for(long x=xmin; x<=xmax; x++){
				...
				if (BlindSelectionIsInside(x,y))
					...
					if (Kfactor==0 || ksumcur==0){
						c.rgbRed   = (BYTE)min(255, max(0,(int)(abs(r) + Koffset)));
						c.rgbGreen = (BYTE)min(255, max(0,(int)(abs(g) + Koffset)));
						c.rgbBlue  = (BYTE)min(255, max(0,(int)(abs(b) + Koffset)));
					} else if (ksumtot == ksumcur) {
						c.rgbRed   = (BYTE)min(255, max(0,(int)(abs(r)/Kfactor + Koffset)));
						c.rgbGreen = (BYTE)min(255, max(0,(int)(abs(g)/Kfactor + Koffset)));
						c.rgbBlue  = (BYTE)min(255, max(0,(int)(abs(b)/Kfactor + Koffset)));
					} else {
						c.rgbRed   = (BYTE)min(255, max(0,(int)((abs(r)*ksumtot)/(ksumcur*Kfactor) + Koffset)));
						c.rgbGreen = (BYTE)min(255, max(0,(int)((abs(g)*ksumtot)/(ksumcur*Kfactor) + Koffset)));
						c.rgbBlue  = (BYTE)min(255, max(0,(int)((abs(b)*ksumtot)/(ksumcur*Kfactor) + Koffset)));
					}
...
bool CxImage::GradientMorpho(long nDiam, bool bHSV)
{
	if (!pDib) 
		return false;
	CxImage tmpHSV (*this);
	if (bHSV){
		if (!tmpHSV.ConvertColorSpace (1, 0))
			return false;
	}
	if (!GrayScale())
		return false;
	...
	long xmin,xmax,ymin,ymax;
	bool bValidSel = SelectionIsValid();
	if (bValidSel){
		xmin = info.rSelectionBox.left; xmax = info.rSelectionBox.right;
		ymin = info.rSelectionBox.bottom; ymax = info.rSelectionBox.top;
	} else {
	...
	unsigned char* cPtr2;
	unsigned char* cPtrH;
	int iY, iY2, iY1, iYH, iYH1, iYH2;
	long k2 = nDiam/2;
	long kmax= nDiam-k2;
	cPtr = info.pImage;
	cPtr2 = (unsigned char *)tmp.info.pImage;
	cPtrH = (unsigned char *)tmpHSV.info.pImage;
	if (bHSV){
		for(long y=ymin; y<=ymax; y++){
			info.nProgress = (long)(100*(y-ymin)/(ymax-ymin));
			if (info.nEscape) 
				break;
			iY1 = y*info.dwEffWidth+xmin * 3;
			iYH1 = y*tmpHSV.info.dwEffWidth + xmin * 3;
			for(long x=xmin; x<=xmax; x++, iY1++){
#if CXIMAGE_SUPPORT_SELECTION
				if (!bValidSel || BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					iY2 = ((y-k2)*info.dwEffWidth);
					iYH2 = ((y-k2)*tmpHSV.info.dwEffWidth);
					for(long j=-k2;j<kmax;j++, iY2+=info.dwEffWidth)
					{
						if (0>(y+j) || (y+j)>=head.biHeight) 
							continue;
						iY = iY2 + x * 3;
						iYH = iYH2 + x * 3;
						BYTE *pDst = cPtr2 + iY1;
						for(long k=-k2;k<kmax;k++)
						{
							if (0>(x+k) || (x+k)>=head.biWidth) 
								continue;
							if (sqrt ((float)(j*j + k*k)) <= k2){
								BYTE curGrad = max (*pDst, (BYTE)abs ((int)cPtrH[iYH+k] - (int)cPtrH[iYH1]));
								pDst[0] = curGrad;
								pDst[1] = curGrad;
								pDst[2] = curGrad;
							}
	...
	else
	{
		for(long y=ymin; y<=ymax; y++){
			...
			if (info.nEscape) 
				break;
			iY1 = y*info.dwEffWidth+xmin * 3;
			for(long x=xmin; x<=xmax; x++, iY1+=3){
#if CXIMAGE_SUPPORT_SELECTION
				if (!bValidSel || BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					...
					for(long j=-k2;j<kmax;j++, iY2+=info.dwEffWidth)
					{
						...
						for(long k=-k2;k<kmax;k++)
						{
							if (0>(x+k) || (x+k)>=head.biWidth) 
								continue;
							if (sqrt ((float)(j*j + k*k)) <= k2){
								BYTE curGrad = max (*pDst, (BYTE)abs ((int)(cPtr[iY+k*3] - (int)cPtr[iY1])));
								pDst[0] = curGrad;
								pDst[1] = curGrad;
								pDst[2] = curGrad;
							}
...
void CxImage::Mix(CxImage & imgsrc2)
{
	...
	unsigned long ulDstWidth = GetWidth();
	unsigned long ulDstHeight = GetHeight();
	long lSrcXOffset = imgsrc2.info.xOffset - info.xOffset;
	long lSrcYOffset = imgsrc2.info.yOffset - info.yOffset;
	unsigned long ulWide = (unsigned long)max ((long)ulDstWidth, lSrcXOffset + (long)ulSrcWidth) - min (0L, lSrcXOffset);
    unsigned long ulHeight = (unsigned long)max ((long)ulDstHeight, lSrcYOffset + (long)ulSrcHeight) - min (0L, lSrcYOffset);
	float fAlphaSrc = 1.0f;
	float fAlphaOpSrc = 0.0f;
	float fAlphaDst = 0.0f;

	if ((ulWide == ulDstWidth) && (ulHeight == ulDstHeight))
	{
		if (imgsrc2.AlphaIsValid())
		{
			if (!AlphaIsValid()) 
				AlphaCreate();

			// No ReAlloc, Alpha
    		BYTE* iDst  = info.pImage + (ulDstHeight - (lSrcYOffset + ulSrcHeight)) * info.dwEffWidth + lSrcXOffset * 3;
			BYTE* iSrc  = imgsrc2.GetBits();		
			BYTE* iAlphaDst = pAlpha + (ulDstHeight - (lSrcYOffset + ulSrcHeight)) * ulDstWidth + lSrcXOffset;
			BYTE* iAlphaSrc = imgsrc2.AlphaGetPointer();
			...
			if (info.hProgress)
			{
				...
				{
					::SendMessageW (info.hProgress, UM_PROGRESS_STEPIT, 2, 0);
					for(unsigned long lX=0;lX<ulSrcWidth;lX++)
					{
						if (*iAlphaSrc)
						{
					...
					iDst += nOffsetNextDstLine;
					iSrc += nOffsetNextSrcLine;
					iAlphaDst += nOffsetNextDstAlphaLine;
				}
			}
			else
			{
		...
		else
		{
			...
			BYTE* iDst  = info.pImage + (ulDstHeight - (lSrcYOffset + ulSrcHeight)) * info.dwEffWidth + lSrcXOffset * 3;
			BYTE* iSrc  = imgsrc2.GetBits();		
			unsigned long ulSrcLineNbBytes = 3 * ulSrcWidth;
			if (info.hProgress)
			{
				for(unsigned long lY=0;lY<ulSrcHeight;lY++)
				{
					::SendMessageW (info.hProgress, UM_PROGRESS_STEPIT, 2, 0);
					memcpy (iDst, iSrc, ulSrcLineNbBytes);
	...
	else
	{
		if (imgsrc2.AlphaIsValid())
		{
			...
			// ReAlloc, Alpha
			DWORD dwDstOrigEffWidth = info.dwEffWidth;
			info.dwEffWidth = ((((24 * ulWide) + 31) / 32) * 4);
			head.biSizeImage = info.dwEffWidth * ulHeight;
			head.biWidth = ulWide;
			head.biHeight = ulHeight;
			DWORD dwSizeDib = head.biSizeImage + head.biSize + GetPaletteSize();

			BYTE *pDstDib = new BYTE [dwSizeDib];		
			BYTE *pDstAlpha = new BYTE [ulWide * ulHeight];		
			memcpy_s (pDstDib, dwSizeDib, &head, head.biSize);
			memcpy_s ((BYTE*)pDstDib + head.biSize, dwSizeDib, (BYTE*)pDib + head.biSize, GetPaletteSize());
			info.pImage = pDstDib + head.biSize + GetPaletteSize();
			ZeroMemory (info.pImage, head.biSizeImage);
			ZeroMemory (pDstAlpha, ulWide * ulHeight);				
			
			// first copy the Dst to the new image
			long lDstXOffset = lSrcXOffset < 0 ? lSrcXOffset : 0;
			long lDstYOffset = lSrcYOffset < 0 ? lSrcYOffset : 0;	
			BYTE* iDst  = info.pImage + (ulHeight - (ulDstHeight - lDstYOffset)) * info.dwEffWidth - lDstXOffset * 3;
			BYTE* iSrc  = GetBits();
			BYTE* iAlphaDst = pDstAlpha + (ulHeight - (ulDstHeight - lDstYOffset)) * ulWide - lDstXOffset;
			BYTE* iAlphaSrc = pAlpha;
			long nOffsetNextDstLine = (long)info.dwEffWidth - (long)ulDstWidth * 3;
			long nOffsetNextSrcLine = (long)dwDstOrigEffWidth - (long)ulDstWidth * 3;
			long nOffsetNextDstAlphaLine = (long)ulWide - (long)ulDstWidth;
			if (info.hProgress)
			{
				for(unsigned long lY=0;lY<ulDstHeight;lY++)
				{
					ProgressStepIt();		 
					for(unsigned long lX=0;lX<ulDstWidth;lX++)
					{
						if (*iAlphaSrc)
						{
							*iDst++ = (BYTE) (*iSrc++);
			...
			// then mix the Src to the new image
			ulDstWidth = ulWide;
			ulDstHeight = ulHeight;
			lSrcXOffset = lSrcXOffset < 0 ? 0 : lSrcXOffset;
			lSrcYOffset = lSrcYOffset < 0 ? 0 : lSrcYOffset;
			iDst  = info.pImage + (ulDstHeight - (lSrcYOffset + ulSrcHeight)) * info.dwEffWidth + lSrcXOffset * 3;
			iSrc  = imgsrc2.GetBits();
			iAlphaDst = pAlpha + (ulDstHeight - (lSrcYOffset + ulSrcHeight)) * ulDstWidth + lSrcXOffset;
			iAlphaSrc = imgsrc2.AlphaGetPointer();
			nOffsetNextDstLine = (long)info.dwEffWidth - (long)ulSrcWidth * 3;
			nOffsetNextSrcLine = (long)dwSrcEffWidth - (long)ulSrcWidth * 3;
			nOffsetNextDstAlphaLine = (long)ulDstWidth - (long)ulSrcWidth;
			if (info.hProgress)
			{
				for(unsigned long lY=0;lY<ulSrcHeight;lY++)
				{
		...
		else
		{
			...
			BYTE* iAlphaDst = pDstAlpha + (ulHeight - (ulDstHeight - lDstYOffset)) * ulWide - lDstXOffset;
			BYTE* iSrc  = GetBits();
			unsigned long ulDstLineNbBytes = 3 * ulDstWidth;
			if (AlphaIsValid())
			{
				BYTE* iAlphaSrc = pAlpha;
				if (info.hProgress)
				{
					...
					{
						ProgressStepIt();	
						memcpy (iDst, iSrc, ulDstLineNbBytes);
						memcpy (iAlphaDst, iAlphaSrc, ulDstWidth);
						iDst += info.dwEffWidth;
						iAlphaDst += ulWide;
						iSrc += dwDstOrigEffWidth;
						iAlphaSrc += ulDstWidth;
					}
				}
				else
				{
			...
			else
			{
				if (info.hProgress)
				{
					...
					{						
						ProgressStepIt();	
						memcpy (iDst, iSrc, ulDstLineNbBytes);
						memset (iAlphaDst, 0xFF, ulDstWidth);
						iDst += info.dwEffWidth;
						iAlphaDst += ulWide;
						iSrc += dwDstOrigEffWidth;
					}
			...
			if (info.hProgress)
			{
				for(unsigned long lY=0;lY<ulSrcHeight;lY++)
				{
					ProgressStepIt();		 
					memcpy (iDst, iSrc, ulSrcLineNbBytes);
					memset (iAlphaDst, 0xFF, ulSrcWidth);
					iDst += info.dwEffWidth;
					iAlphaDst += ulDstWidth;
					iSrc += dwSrcEffWidth;
				}
			}
			else
		...
		info.xOffset = imgsrc2.info.xOffset < info.xOffset ? imgsrc2.info.xOffset : (info.xOffset < 0 ? info.xOffset : 0);
		info.yOffset = imgsrc2.info.yOffset < info.yOffset ? imgsrc2.info.yOffset : (info.yOffset < 0 ? info.yOffset : 0);
	}
}
#endif
				   ...
				   long direction, bool bForceFFT, bool bMagnitude)
{
	...
	for (j=0;j<h;j++) {
		for (k=0;k<w;k++) {
			if (bMagnitude){
				tmpReal->SetPixelIndex(k,j,(BYTE)max(0L,min(255L,(LONG)(nn*(3+log(_cabs(grid[k][j])))))));
				if (grid[k][j].x==0){
					tmpImag->SetPixelIndex(k,j,(BYTE)max(0L,min(255L,(LONG)(128L+(atan(grid[k][j].y/0.0000000001)*nn)))));
				} else {
					tmpImag->SetPixelIndex(k,j,(BYTE)max(0L,min(255L,(LONG)(128L+(atan(grid[k][j].y/grid[k][j].x)*nn)))));
				}
			} else {
				tmpReal->SetPixelIndex(k,j,(BYTE)max(0L,min(255L,(LONG)(128L + grid[k][j].x*nn))));
				tmpImag->SetPixelIndex(k,j,(BYTE)max(0L,min(255L,(LONG)(128L + grid[k][j].y*nn))));
			}
		}
	}
	...
	for(x=1; x<w; x++){
		for(y=1; y<h; y++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				tmp.BlindSetPixelIndex(x,y,(BYTE)min(255L,max(0L,(LONG)(xy0 + radius * correction + 0.5))));
			}
		}
	}
...
/** Called after each pixel modification */
void CxImage::UpdateImageParameters()
{
	if (info.nNumLayers == 0)
	{
		BYTE* iSrc  = GetBits();
		long nNbPixels = head.biWidth * head.biHeight;
		...
		if (nBlueValue < nRedValue && nBlueValue < nGreenValue)
			m_cSelection = 0xFF0000;
		else if (nRedValue < nGreenValue)
			m_cSelection = 0xFF;		
		else
			m_cSelection = 0xFF00;
	}
...
bool CxImage::GaussianBlur(float radius /*= 1.0f*/, CxImage* iDst /*= 0*/)
{
	...
	if (!tmp_x.IsValid()){
		strcpy(info.tcLastError,tmp_x.GetLastError());
		return false;
	}
	...
	if (!tmp_y.IsValid()){
		strcpy(info.tcLastError,tmp_y.GetLastError());
		return false;
	}
...
bool CxImage::SelectiveBlur(float radius, BYTE threshold, CxImage* iDst)
{
	...
	if (!Dst.IsValid()){
		strcpy(info.tcLastError,Dst.GetLastError());
		return false;
	}
 ...
 * \return true if everything is ok
 */
bool CxImage::RedEyeRemove (RECT rcLeft, RECT rcRight, float strength)
{
	if (!pDib) return false;
	...
	float fEYoX= 0.5f;
	IGSTRUCTELEM_ELLIPSE eyeEllipse (&fEYoX);
	POINT ptEyeLeftCtr,ptEyeRightCtr;
	ptEyeLeftCtr.x = (rcLeft.left + rcLeft.right) / 2;
	ptEyeLeftCtr.y = (rcLeft.top + rcLeft.bottom) / 2;
	ptEyeRightCtr.x = (rcRight.left + rcRight.right) / 2;
	ptEyeRightCtr.y = (rcRight.top + rcRight.bottom) / 2;
	long nEyeRay = (long)(0.5f*(float)max(rcRight.bottom - rcRight.top, rcLeft.bottom - rcLeft.top));
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				RGBQUAD colHsv = hsv.BlindGetPixelColor(x,y);
				float fDistRatio;
				if (rcLeft.right == -1 || rcLeft.bottom == -1 || rcRight.right == -1 || rcRight.bottom == -1)
					fDistRatio = 1.0f;
				else {
					float evlEyeL=min(1.0f,eyeEllipse(x - ptEyeLeftCtr.x, y - ptEyeLeftCtr.y)/(float)nEyeRay);
					float evlEyeR=min(1.0f,eyeEllipse(x - ptEyeRightCtr.x, y - ptEyeRightCtr.y)/(float)nEyeRay);
					fDistRatio = evlEyeL * evlEyeR;
				}
				if ((colHsv.rgbRed > 223 || colHsv.rgbRed < 32) && colHsv.rgbGreen > 50 && colHsv.rgbBlue > 50 && fDistRatio < 1.0f) {
					float a = (1.0f - fDistRatio) * 2.0f * strength; 
					if (colHsv.rgbGreen < 150)
						a *= (float)(colHsv.rgbGreen - 50) / 100.f;
					if (colHsv.rgbRed < 239 && colHsv.rgbRed > 16){
						if (colHsv.rgbRed < 32)
							a *= (float)(colHsv.rgbRed - 16) / 16.0f;
						else
							a *= (float)(239 - colHsv.rgbRed) / 16.0f;
					}
					a = min (a, 1.0f);
					if (a > 0) {
...
class PredIrisDetectMore
{
public:
	bool operator () (const pair<POINT, int>& pt1, const pair<POINT, int>& pt2)
	{
...
public:
	bool operator () (const pair<POINT, int>& pt1, const pair<POINT, int>& pt2)
	{
		if (pt1.first.y < pt2.first.y)
			return true;
		else if (pt1.first.y > pt2.first.y)
			return false;
...
public:
	PredIsInList () {}
	bool operator () (const pair<POINT, int>& pt) {
		if (m_lPts.find (pt) != m_lPts.end())
			return true;
		m_lPts.insert (pt);
		return false;
	}
private:
	set <pair <POINT, int>, PredPtSetLess> m_lPts;
};

POINT CxImage::GetEyeCenter()
{
	POINT ptRet = {-1,-1};
	if (head.biBitCount != 8)
		return ptRet;
	int nRay = (int)(head.biHeight / 4);
	int nMaxNbPx = -1;
	POINT ptCtr;
	ptCtr.x = head.biWidth / 2;
	ptCtr.y = head.biHeight / 2;
	list <pair <POINT, int>> lHighestPts;
	for (long y = 0; y < head.biHeight; y++){
		for (long x = 0; x < head.biWidth; x++){
			float fCurNbPx = 0.0f;
			POINT ptCur;
			ptCur.x = x;
			ptCur.y = y;
			float fCoeff = ((float)head.biWidth - abs((float)ptCur.x - (float)ptCtr.x)) / (float)ptCtr.x;
			for (long yoff = max(0L,y-nRay); yoff < min(head.biHeight,y+nRay); yoff++){
				BYTE *pBits = GetBits (yoff) + max(0L,x - nRay);
				for (long xoff = max(0L,x - nRay); xoff < min(head.biWidth,x + nRay); xoff++){
					if ((((xoff-x)*(xoff-x) + (yoff-y)*(yoff-y)) <= nRay * nRay) && (*pBits == 0))
						fCurNbPx += fCoeff;
					pBits++;
				}
			}
			lHighestPts.push_back (pair <POINT, int> (ptCur, (int)fCurNbPx));
		}
	}
	PredIrisDetectMore predIrisDetectMore;
	lHighestPts.sort (predIrisDetectMore);
	return lHighestPts.front().first;
}

bool CxImage::GetEyeCenters (RECT rcLeft, RECT rcRight, POINT& ptCenterLeft, POINT& ptCenterRight)
{
	CxImage eyeLeft (*this, true, false, false);
	rcLeft.top += (rcLeft.bottom - rcLeft.top) / 4;
	rcLeft.bottom -= (rcLeft.bottom - rcLeft.top) / 4;
	rcLeft.left += (rcLeft.right - rcLeft.left) / 4;
	if (!eyeLeft.Crop (rcLeft.left, head.biHeight - rcLeft.bottom, rcLeft.right, head.biHeight - rcLeft.top))
		return false;
	int nHeight = rcLeft.bottom - rcLeft.top;
	long gray[256];
	CxImage eyeLeftMsk (eyeLeft, true, false, false);
	eyeLeftMsk.GrayScaleRed();
	eyeLeftMsk.Threshold ((BYTE)eyeLeftMsk.Histogram(0,0,0,gray));
	eyeLeftMsk.Erode(nHeight / 4);
	eyeLeftMsk.Dilate(nHeight / 4);
	eyeLeftMsk.Negative();
	//eyeLeftMsk.Save(L"eyeLeftMsk.jpg", CXIMAGE_FORMAT_JPG);
	eyeLeft.Threshold ((BYTE)eyeLeft.Histogram(0,0,0,gray));
	//eyeLeft.Save(L"eyeLeftThresh.jpg", CXIMAGE_FORMAT_JPG);
	eyeLeft.Erode(nHeight / 4);
	eyeLeft.Dilate(nHeight / 4);
	//eyeLeft.Save(L"eyeLeftElStr.jpg", CXIMAGE_FORMAT_JPG);
	eyeLeft.ThresholdAnd (&eyeLeftMsk, 255);
	//eyeLeft.Save(L"eyeLeft.jpg", CXIMAGE_FORMAT_JPG);
	ptCenterLeft = eyeLeft.GetEyeCenter();
	ptCenterLeft.x += (rcLeft.right - rcLeft.left) / 4;
	ptCenterLeft.y += (rcLeft.bottom - rcLeft.top) / 4;
	CxImage eyeRight (*this, true, false, false);
	rcRight.top += (rcRight.bottom - rcRight.top) / 4;
	rcRight.bottom -= (rcRight.bottom - rcRight.top) / 4;
	rcRight.right -= (rcLeft.right - rcLeft.left) / 4;
	if (!eyeRight.Crop (rcRight.left, head.biHeight - rcRight.bottom, rcRight.right, head.biHeight - rcRight.top))
		return false;
	nHeight = rcRight.bottom - rcRight.top;
	CxImage eyeRightMsk (eyeRight, true, false, false);
	eyeRightMsk.GrayScaleRed();
	eyeRightMsk.Threshold ((BYTE)eyeRightMsk.Histogram(0,0,0,gray));
	eyeRightMsk.Erode(nHeight / 4);
	eyeRightMsk.Dilate(nHeight / 4);
	eyeRightMsk.Negative();
	//eyeRightMsk.Save(L"eyeRightMsk.jpg", CXIMAGE_FORMAT_JPG);
	eyeRight.Threshold ((BYTE)eyeRight.Histogram(0,0,0,gray));
	//eyeRight.Save(L"eyeRightThresh.jpg", CXIMAGE_FORMAT_JPG);
	eyeRight.Erode(nHeight / 4);
	eyeRight.Dilate(nHeight / 4);
	//eyeRight.Save(L"eyeRightElStr.jpg", CXIMAGE_FORMAT_JPG);
	eyeRight.ThresholdAnd (&eyeRightMsk, 255);
	//eyeRight.Save(L"eyeRight.jpg", CXIMAGE_FORMAT_JPG);
	ptCenterRight = eyeRight.GetEyeCenter();
	ptCenterRight.y += (rcRight.bottom - rcRight.top) / 4;
	return true;
}
		...
		for(long x=xmin; x<=xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				if (rcLeft.right == -1 || rcLeft.bottom == -1 || rcRight.right == -1 || rcRight.bottom == -1)
					...
					float fDistCtrLeft = sqrtf ((float)((x - xCtrLeft) * (x - xCtrLeft) + (y - yCtrLeft) * (y - yCtrLeft)));
					float fDistCtrRight = sqrtf ((float)((x - xCtrRight) * (x - xCtrRight) + (y - yCtrRight) * (y - yCtrRight)));
					if (fDistCtrLeft < fDistCtrRight)
						fDistRatio = fDistCtrLeft / ((float)sizeLeft / 2.0f);
					else
						fDistRatio = fDistCtrRight / ((float)sizeRight / 2.0f);
				}
				fDistRatio *= 3.0f / fStrength;
				if (fDistRatio < 0.1f)
					fDistRatio = 0.1f;
				if (fDistRatio < 1.0f) {
					RGBQUAD colHsv = hsv.BlindGetPixelColor (x, y);
					colHsv.rgbRed = hue;
					colHsv.rgbGreen = sat;
					RGBQUAD colRGB = HSLtoRGB (colHsv);
					RGBQUAD color = BlindGetPixelColor (x, y);
					color.rgbRed = (BYTE)(colRGB.rgbRed * (1.0f - fDistRatio) + color.rgbRed * fDistRatio);
					color.rgbGreen = (BYTE)(colRGB.rgbGreen * (1.0f - fDistRatio) + color.rgbGreen * fDistRatio);
					color.rgbBlue = (BYTE)(colRGB.rgbBlue * (1.0f - fDistRatio) + color.rgbBlue * fDistRatio);
					BlindSetPixelColor (x, y, color);
				}
			}
		}
...
bool CxImage::Saturate(const long saturation, const long colorspace)
{
	...
	case 2:
		...
		break;
	default:
		strcpy(info.tcLastError,"Saturate: wrong colorspace");
		return false;
	}
	return true;
}
	...
	for (long y=0;y<h;y++){
		...
		for (long x=0;x<w;x++){
			...
			case 4:
				...
				break;
			default:
				strcpy(info.tcLastError,"ConvertColorSpace: unknown source colorspace");
				return false;
			}
			...
			case 4:
				...
				break;
			default:
				strcpy(info.tcLastError,"ConvertColorSpace: unknown destination colorspace");
				return false;
			}
			BlindSetPixelColor(x,y,c);
		}
...
int  CxImage::OptimalThreshold(long method, RECT * pBox, CxImage* pContrastMask)
{
	...
	if (head.biBitCount!=8){
		strcpy(info.tcLastError,"OptimalThreshold works only on 8 bit images");
		return -1;
	}
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			strcpy(info.tcLastError,"OptimalThreshold invalid ContrastMask");
			return -1;
		}
	}
	...
	long xmin,xmax,ymin,ymax;
	if (pBox){
		xmin = max(pBox->left,0L);
		xmax = min(pBox->right,head.biWidth);
		ymin = max(pBox->bottom,0L);
...
bool CxImage::AdaptiveThreshold(long method, long nBoxSize, CxImage* pContrastMask, long nBias, float fGlobalLocalBalance)
{
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			pContrastMask->GetWidth() != GetWidth() ||
			pContrastMask->GetHeight() != GetHeight()){
			strcpy(info.tcLastError,"AdaptiveThreshold invalid ContrastMask");
			return false;
		}
	}
	...
	for (long y=0; y<mh; y++){
		for (long x=0; x<mw; x++){
			...
			mask.SetPixelIndex(x,y,(BYTE)max(0L,min(255L,(LONG)(nBias+((1.0f-fGlobalLocalBalance)*threshold + fGlobalLocalBalance*globalthreshold)))));
		}
	}
						...
						BYTE nOpacity, const bool bSelectFilledArea, const BYTE nSelectionLevel)
{
	...
	BYTE* pFillMask = (BYTE*)calloc(1, head.biWidth * head.biHeight);
	if (!pFillMask)
		return false;
...
bool CxImage::FillGradient(const long xStart, const long yStart, const long nDirectionX, const long nDirectionY, const RGBQUAD cFillColor, const RGBQUAD cFillColor2)
{
	if (!pDib)
		return false;
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				float fDirectionSize = sqrtf ((float)(nDirectionY * nDirectionY + nDirectionX * nDirectionX));
				float fScalProd = ((float)(nDirectionX * (x - xStart) + nDirectionY * (y - yStart))) / fDirectionSize;
				if (fScalProd < 0.0f)
					fScalProd = 0.0f;
				if (fScalProd > fDirectionSize)
					fScalProd = fDirectionSize;
				fScalProd /= fDirectionSize;
				*pPxl++ = (BYTE)((float)cFillColor.rgbRed * (1.0f - fScalProd) + (float)cFillColor2.rgbRed * fScalProd);
				*pPxl++ = (BYTE)((float)cFillColor.rgbGreen * (1.0f - fScalProd) + (float)cFillColor2.rgbGreen * fScalProd);
				*pPxl++ = (BYTE)((float)cFillColor.rgbBlue * (1.0f - fScalProd) + (float)cFillColor2.rgbBlue * fScalProd);
			}
		}
	}
	return true;
...
CxImage& CxImage::operator += (CxImage& addImage)
{
	if ((head.biBitCount==8) && IsGrayScale())
	{
		for(long y=0; y<addImage.head.biHeight; y++){
			BYTE* pFrom = addImage.GetBits(y);
			BYTE* pDst = GetBits(y);
			for(long x=0; x<addImage.head.biWidth; x++){
				*pDst++ += *pFrom++;
			}
		}
	}
...
bool CxImage::DeletePixels(bool bSmart)
{
	if (!pDib) return 0;
	...
	BYTE *iDst=tmp.info.pImage;	
	iDst += tmp.info.dwEffWidth*ymin; 
	BYTE *iAlphaDst=tmp.AlphaGetPointer(0, ymin);
	...
	for(long y=ymin; y<=ymax; y++){
		...
		iDst += tmp.info.dwEffWidth;
		iAlphaDst += head.biWidth;
	}
	Transfer (tmp);
	return true;
}

bool CxImage::MovePixels(int nVectorX, int nVectorY)
{
	if (!pDib) return 0;
	...
	if (xmin + nVectorX >= head.biWidth || 
		xmax + nVectorX < 0 ||
		ymin + nVectorY >= head.biHeight || 
		ymax + nVectorY < 0)
		return false;

	BYTE *iSrc = info.pImage + info.dwEffWidth * ymin; 
	BYTE *iDstFrom = tmp.info.pImage + info.dwEffWidth * ymin;	
	BYTE *iDst = tmp.info.pImage + info.dwEffWidth * (ymin + nVectorY);	
	BYTE *iAlphaSrc = AlphaGetPointer (0, ymin);
	BYTE *iAlphaDstFrom = tmp.AlphaGetPointer (0, ymin);
	BYTE *iAlphaDst = tmp.AlphaGetPointer (0, ymin + nVectorY);
	BYTE *iSelSrc = SelectionGetPointer (0, ymin);
	BYTE *iSelDstFrom = tmp.SelectionGetPointer (0, ymin);
	BYTE *iSelDst = tmp.SelectionGetPointer (0, ymin + nVectorY);
	for(long y=ymin; y<=ymax; y++){
		info.nProgress = (long)(50*(y-ymin)/(ymax-ymin)); 
		for(long x=xmin; x<=xmax; x++){
			if (BlindSelectionIsInside (x, y)){				
				BYTE *pPxlDstFrom = iDstFrom + x * 3;
				*pPxlDstFrom++ = CXIMAGE_GRAY;
				*pPxlDstFrom++ = CXIMAGE_GRAY;
				*pPxlDstFrom = CXIMAGE_GRAY;
				iAlphaDstFrom[x] = 0;
				iSelDstFrom[x] = 0;	
		...
		iDstFrom += info.dwEffWidth;
		iDst += info.dwEffWidth;
		iAlphaSrc += head.biWidth;
		iAlphaDstFrom += head.biWidth;
		iAlphaDst += head.biWidth;
		iSelSrc += head.biWidth;
		iSelDstFrom += head.biWidth;
		iSelDst += head.biWidth;
	}

	iSrc = info.pImage + info.dwEffWidth * ymin; 
	iDstFrom = tmp.info.pImage + info.dwEffWidth * ymin;	
	iDst = tmp.info.pImage + info.dwEffWidth * (ymin + nVectorY);	
	iAlphaSrc = AlphaGetPointer (0, ymin);
	iAlphaDstFrom = tmp.AlphaGetPointer (0, ymin);
	iAlphaDst = tmp.AlphaGetPointer (0, ymin + nVectorY);
	iSelSrc = SelectionGetPointer (0, ymin);
	iSelDstFrom = tmp.SelectionGetPointer (0, ymin);
	iSelDst = tmp.SelectionGetPointer (0, ymin + nVectorY);
	for(long y=ymin; y<=ymax; y++){
		info.nProgress = 50L + (long)(50*(y-ymin)/(ymax-ymin)); 
		for(long x=xmin; x<=xmax; x++){
			if (BlindSelectionIsInside (x, y)){	
				if (IsInside (x + nVectorX, y + nVectorY)){
					BYTE *pPxlSrc = iSrc + x * 3;
					BYTE *pPxlDst = iDst + (x + nVectorX) * 3;
					*pPxlDst++ = *pPxlSrc++;
					*pPxlDst++ = *pPxlSrc++;
					*pPxlDst = *pPxlSrc;
					iAlphaDst[x + nVectorX] = iAlphaSrc[x];
					iSelDst[x + nVectorX] = 255;		
				}
			}
		}
		iSrc += info.dwEffWidth;
