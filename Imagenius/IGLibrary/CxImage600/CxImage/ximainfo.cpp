...
void CxImage::SetTransMode(ENUM_CXIMAGE_TRANSMODE transMode)
{
	info.nTransMode = transMode;
}
////////////////////////////////////////////////////////////////////////////////
...
// forcing same density in X and Y for deepzoom compatibility
 */
long CxImage::GetXDPI() const
{
	return max (info.xDPI, info.yDPI);
}
////////////////////////////////////////////////////////////////////////////////
/**
 * \return Resolution for TIFF, JPEG, PNG and BMP formats.
...
////////////////////////////////////////////////////////////////////////////////
void CxImage::SetHeight(DWORD dwHeight)
{
	head.biHeight = dwHeight;
}
////////////////////////////////////////////////////////////////////////////////
void CxImage::SetWidth(DWORD dwWidth)
{
	head.biWidth = dwWidth;
}

void CxImage::SetSize(int nWidth, int nHeight)  
{
	head.biWidth = nWidth;
...
DWORD CxImage::GetTypeIndexFromId(const DWORD id)
{
	...
	n++; if (id == CXIMAGE_FORMAT_IG) return n;
#endif
#if CXIMAGE_SUPPORT_DZ
	n++; if (id == CXIMAGE_SUPPORT_DZ) return n;
#endif
#if CXIMAGE_SUPPORT_TGA
...
float CxImage::GetCurrentZoom() const
{
	return info.fZoomCurrent;
}

void CxImage::SetCurrentZoom (float fCurrentZoom)
{
	if ((fCurrentZoom <= 0.0f) || (fCurrentZoom > IGFRAME_MAXZOOM))
		return;
	info.fZoomCurrent = fCurrentZoom;
}
...
DWORD CxImage::UnDump(const BYTE * src)
{
	...
	if (src[n++]){
		for (long m=0; m<GetNumLayers(); m++){
			CxImage *pLayer = new CxImage();
			n += pLayer->UnDump(&src[n]);
