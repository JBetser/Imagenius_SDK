...
{
	strcpy(info.tcLastError,message);
...
bool CxImagePNG::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
    ...
    * set up your own error handlers in the png_create_read_struct() earlier. */
	...
	png_set_error_fn(png_ptr,info.tcLastError,/*(png_error_ptr)*/user_error_fn,NULL);
	...
	ProgressSetRange (info_ptr->height, 0);
	ProgressSetMessage (L"Decoding PNG...");	
	...
	case PNG_COLOR_TYPE_RGB_ALPHA:
		...
		break;
	default:
		strcpy(info.tcLastError,"unknown PNG color type");
		longjmp(png_ptr->jmpbuf, 1);
	}
	...
	if (strcmp(message,"")) strncpy(info.tcLastError,message,255);
...
bool CxImagePNG::Encode(CxFile *hFile)
{
	...
	_ASSERTE (info.hProgress && L"Missing progress bar");
