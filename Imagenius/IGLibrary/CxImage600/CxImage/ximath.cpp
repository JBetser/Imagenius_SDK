...
float CxMath::NormalDistrib(float mean, float stdDev, float x, bool bNormalised, float fCoeff)
{
	if (bNormalised)
		return expf (-pow((x - mean)/stdDev, 2.0f) / 2.0f) / (stdDev * sqrtf (2.0f * PI));
	return expf (-pow((x - mean)/stdDev, 2.0f) / (2.0f * fCoeff));
}