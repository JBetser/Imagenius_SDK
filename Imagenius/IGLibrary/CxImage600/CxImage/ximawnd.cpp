...
void getRGBColor (IGLibrary::IGSmartPixel *pSmartPixel, BYTE& r, BYTE& g, BYTE& b)
{
	if (pSmartPixel->m_nLabel == -1){
		r=0;
		g=0;
		b=0;
	}
	else if (pSmartPixel->m_nLabel == 0){
		r=0;
		g=255;
		b=0;
	}
	else if (pSmartPixel->m_bEyes){
		r = 255;
		g = 0;
		b = 255;
	}
	else{
		r = (BYTE)(255-(pSmartPixel->m_nLabel*20)%255);
		g = (BYTE)((pSmartPixel->m_nLabel*170)%255);
		b = (BYTE)(127-(pSmartPixel->m_nLabel*35)%255*(int)powf(-1.0f,(float)pSmartPixel->m_nLabel));
	}
}
...
bool CxImage::CreateFromHANDLE(HANDLE hMem)
{
	...
	if (lpVoid){
		...
		// If there is an alpha channel in this handle, we import it
		if (dwSize == GetSize() + head.biWidth * head.biHeight)
		{
			AlphaSetChannel ((BYTE*)lpVoid + GetSize());
		}
 ...
 */
HBITMAP CxImage::MakeBitmap (HDC hdc, int nImageWidth, int nImageHeight, bool bDrawChessBackground, bool bDrawWhiteBackground, bool bMixAlpha)
{
	GdiFlush ();

	void *pSrcDib = (info.nNumLayers > 0 ? GetLayer(0)->GetDIB() : pDib);
	if (!pSrcDib)
		return NULL;
	if ((nImageWidth < 0) || (nImageHeight < 0))
	{
		...
		{
			if (nImageWidth < 1)
				nImageWidth = GetLayer(0)->GetWidth();
			if (nImageHeight < 1)
				nImageHeight = GetLayer(0)->GetHeight();
		}
		else
		{
			nImageWidth = head.biWidth;
			nImageHeight = head.biHeight;
		}
	}

	HDC hMemDC = CreateCompatibleDC(hdc);
	LPVOID pBit32;
	HBITMAP bmp = ::CreateDIBSection (hMemDC, (LPBITMAPINFO)pSrcDib, DIB_RGB_COLORS, &pBit32, NULL, 0);
	HGDIOBJ hOldBmp = ::SelectObject (hMemDC, bmp);
	if (bDrawWhiteBackground)
	{		
		// draw a white background
		HPEN hWhitePen = ::CreatePen (PS_NULL, 1, RGB (255,255,255));
		HGDIOBJ hOldPen = ::SelectObject (hMemDC, hWhitePen);
		::Rectangle (hMemDC, 0, 0, nImageWidth + 1, nImageHeight + 1);
		::SelectObject (hMemDC, hOldPen);
		::DeleteObject (hWhitePen);
		bDrawChessBackground = false;
	}
	LPRECT lpClipRect = NULL;
	RECT rcClip = {0,0,0,0};
	if (nImageWidth > 0 && nImageHeight > 0)
	{
		rcClip.right = nImageWidth;
		rcClip.bottom = nImageHeight;
		lpClipRect = &rcClip;
	}
	if (info.nNumLayers > 0)
		LayerDrawAll (hMemDC, 0, 0, nImageWidth, nImageHeight, lpClipRect, false, bDrawChessBackground, bMixAlpha);
	else
		Draw (hMemDC, 0, 0, nImageWidth, nImageHeight, lpClipRect, false, bDrawChessBackground, bMixAlpha);
 ...
 * \return true if everything is ok
 */
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal, bool bUpdateParams)
{
	if (!hbmp)
		return false;
    ...
    if (GetDIBits(dc, hbmp, 0, head.biHeight, info.pImage,
		(LPBITMAPINFO)pDib, DIB_RGB_COLORS) == 0)
	{ //replace &head with pDib <Wil Stark>
		strcpy(info.tcLastError,"GetDIBits failed");
		::ReleaseDC(NULL, dc);
		return false;
    }
    ::ReleaseDC(NULL, dc);
...
IGLibrary::IGSmartPixel*	CxImage::GetSegmentationLayer (IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_ENUM& eShowSegmType)
{
	return NULL;
}

////////////////////////////////////////////////////////////////////////////////
long CxImage::Draw(HDC hdc, const RECT& rect, RECT* pClipRect, bool bSmooth, bool bMixAlpha)
{
	return Draw(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, pClipRect, bSmooth, bMixAlpha);
}
////////////////////////////////////////////////////////////////////////////////
/**
 ...
 * \return true if everything is ok
 */
long CxImage::Draw (HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect, bool bSmooth, bool bDrawChessBackGround, bool bMixAlpha)
{
	// check if bitmap is valid
	if((pDib==0)||(hdc==0)) 
		return 0;
	// if image is disabled or its requested size is null just skip its drawing
	if ((cx==0)||(cy==0)||(!info.bEnabled))
		return 1;	

	if (cx < 0) cx = head.biWidth;
	if (cy < 0) cy = head.biHeight;
	bool bTransparent = info.nBkgndIndex >= 0;
	bool bAlpha = (bMixAlpha && (pAlpha != 0));
	...
	//////////////////////////////////////////////////////////////////
		...
		{
			HGDIOBJ TmpObj = ::SelectObject (TmpDC, TmpBmp);
			if (bDrawChessBackGround)
			{
				HBRUSH hWhiteBrush = ::CreateSolidBrush (RGB (230, 230, 230));
				HPEN hNullPen = ::CreatePen (PS_NULL, 1, RGB (0,0,0));
				HGDIOBJ hOldPen = ::SelectObject (TmpDC, hNullPen);
				HGDIOBJ hOldBrush = ::SelectObject (TmpDC, hWhiteBrush);
				::Rectangle (TmpDC, -1, -1, destw + 1, desth + 1);
				::SelectObject (TmpDC, hOldBrush);
				::DeleteObject (hWhiteBrush);
				HBRUSH hGreyBrush = ::CreateSolidBrush (RGB (200, 200, 200));	
				hOldBrush = ::SelectObject (TmpDC, hGreyBrush);
				int nDrawSizeX = 0;
				int nDrawSizeY = 0;
				int nMaxDrawSizeX = destw + 8;
				int nMaxDrawSizeY = desth + 8;
				bool bDrawX = true;
				bool bDrawY = true;
				while (nDrawSizeX < nMaxDrawSizeX)
				{
					...
					{
						if (bDrawY)	
							::Rectangle (TmpDC, nDrawSizeX - 1, nDrawSizeY - 1, nDrawSizeX + 8, nDrawSizeY + 8);
						bDrawY = !bDrawY;
						nDrawSizeY += 8;
					}
					bDrawX = !bDrawX;
			...
			{
				::BitBlt(TmpDC, 0, 0, destw, desth, 
						hdc, paintbox.left, paintbox.top, SRCCOPY);
			}			
				...
				if (cx!=head.biWidth || cy!=head.biHeight){
					...
					for(yy=0;yy<desth;yy++){
						...
						for(xx=0;xx<destw;xx++){
							dx = (xx+xmin-x)*fx;
							sx = max(0L,(long)floor(dx));
							a = (bAlpha) ? pAlpha[alphaoffset+sx] : 255;
...
long CxImage::DrawWithSelection (HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect, bool bSmooth, bool bDrawChessBackGround, bool bMixAlpha)
{
	if (pSelection==0)
		return CxImage::Draw (hdc, x, y, cx, cy, pClipRect, bSmooth, bDrawChessBackGround, bMixAlpha);
	// check if bitmap is valid
	if((pDib==0)||(hdc==0)) 
		return 0;
	// if image is disabled or its requested size is null just skip its drawing
	...
	BYTE *pIterSelection = pSelection;
//	RGBQUAD rgbqSel;
//	float fRatio;
			...
			if (pbase){
				...
				for(yy=0;yy<desth;yy++){
					...
					for(xx=0;xx<destw;xx++){
						...
						pIterSelection = pSelection + sy * head.biWidth + sx;
						if (*pIterSelection)
						{/*
							rgbqSel = RGBtoHSL (c);
							rgbqSel.rgbRed = (m_cSelection == 0xFF) ? 0x00 : (m_cSelection == 0xFF00) ? 0x40 : 0x80;
							rgbqSel.rgbBlue = (rgbqSel.rgbBlue > 128) ? (BYTE)((int)rgbqSel.rgbBlue / 2) : (BYTE)(((int)rgbqSel.rgbBlue + 255) / 2);
							rgbqSel = HSLtoRGB (RGBQUADtoRGB (rgbqSel));
							*pdst++= rgbqSel.rgbBlue;
							*pdst++= rgbqSel.rgbGreen;
							*pdst++= rgbqSel.rgbRed;*/
							*pdst++= (BYTE)(((int)c.rgbBlue + (int)GetBValue(m_cSelection))>>1); 
							*pdst++= (BYTE)(((int)c.rgbGreen + (int)GetGValue(m_cSelection))>>1); 
							*pdst++= (BYTE)(((int)c.rgbRed + (int)GetRValue(m_cSelection))>>1);	
						}
						else
						{/*
							*pdst++= c.rgbBlue;
							*pdst++= c.rgbGreen;
							*pdst++= c.rgbRed;*/
							*pdst++= (BYTE)(((int)c.rgbBlue + 255)>>1); 
							*pdst++= (BYTE)(((int)c.rgbGreen + 255)>>1); 
							*pdst++= (BYTE)(((int)c.rgbRed + 255)>>1);	
						}
					}
				}
			}
			...
			if (pbase){
				...
				if (cx!=head.biWidth || cy!=head.biHeight){
					...
					for(yy=0;yy<desth;yy++){
						...
						for(xx=0;xx<destw;xx++){
							...
							if ((head.biClrUsed && ci!=cit) || (!head.biClrUsed && *pc!=*pct) || !bTransparent){
								...
								if (*pIterSelection)
								{
									c.rgbBlue = ((int)GetBValue(m_cSelection) + (int)c.rgbBlue) >> 1;
									c.rgbRed = ((int)GetRValue(m_cSelection) + (int)c.rgbRed) >> 1;
									c.rgbGreen = ((int)GetGValue(m_cSelection) + (int)c.rgbGreen) >> 1;
								}
								else
								{
									c.rgbBlue = (255 + (int)c.rgbBlue) >> 1;
									c.rgbRed = (255 + (int)c.rgbRed) >> 1;
									c.rgbGreen = (255 + (int)c.rgbGreen) >> 1;
								}
								/*
								if (*pIterSelection)
								{
									...
									rgbqSel.rgbBlue = (rgbqSel.rgbBlue > 128) ? (BYTE)((int)rgbqSel.rgbBlue / 2) : (BYTE)(((int)rgbqSel.rgbBlue + 255) / 2);
									rgbqSel = HSLtoRGB (RGBQUADtoRGB (rgbqSel));
									if (m_cSelection == 0xFF)
										rgbqSel.rgbRed = 0xFF;
									else if (m_cSelection == 0xFF00)
										rgbqSel.rgbGreen = 0xFF;
									else
										rgbqSel.rgbBlue = 0xFF;
									c = rgbqSel;
								}*/
								if (a == 0) {			// Transparent, retain dest 
									if (*pIterSelection)
									...
									*pdst++= c.rgbRed;
									...
									a1=(BYTE)~a;
									*pdst++= (BYTE)(((int)(*pdst) * (int)a1 + (int)a * (int)c.rgbBlue) >> 8); 
									*pdst++= (BYTE)(((int)(*pdst) * (int)a1 + (int)a * (int)c.rgbGreen) >> 8); 
									*pdst++= (BYTE)(((int)(*pdst) * (int)a1 + (int)a * (int)c.rgbRed) >> 8);
								} 
							} else {
								pdst+=3;
							}
					...
					for(yy=0;yy<desth;yy++,iy++){
						...
						ppix=info.pImage+iy*info.dwEffWidth+ix*3;
						for(xx=0;xx<destw;xx++,ix++){
							a = (bAlpha) ? pAlpha[alphaoffset+ix] : 255;
							a = (BYTE)((a*(1+info.nAlphaMax))>>8);
							...
							if ((head.biClrUsed && ci!=cit) || (!head.biClrUsed && *pc!=*pct) || !bTransparent){
								// DJT, assume many pixels are fully transparent or opaque and thus avoid multiplication
								pIterSelection = pSelection + iy * head.biWidth + ix;	
								if (*pIterSelection)
								{
									c.rgbBlue = ((int)GetBValue(m_cSelection) + (int)c.rgbBlue) >> 1;
									c.rgbRed = ((int)GetRValue(m_cSelection) + (int)c.rgbRed) >> 1;
...
long CxImage::DrawWithSegmentation (HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect, bool bSmooth, bool bDrawChessBackGround, bool bMixAlpha)
{
	IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_ENUM eShowSegmType = IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_NO;
	IGLibrary::IGSmartPixel *pSegmentation = GetSegmentationLayer (eShowSegmType);
	if (eShowSegmType == IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_NO || !pSegmentation)
		return CxImage::Draw (hdc, x, y, cx, cy, pClipRect, bSmooth, bDrawChessBackGround, bMixAlpha);
	// check if bitmap is valid
	if((pDib==0)||(hdc==0)) 
		return 0;
	...
	IGLibrary::IGSmartPixel *pIterSegmentation = pSegmentation;
			...
			if (pbase){
				...
				for(yy=0;yy<desth;yy++){
					...
					for(xx=0;xx<destw;xx++){
						...
						pIterSegmentation = pSegmentation + sy * head.biWidth + sx;
						getRGBColor (pIterSegmentation, c.rgbRed, c.rgbGreen, c.rgbBlue);
						if ((eShowSegmType & IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT) == IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT){
							*pdst++= (BYTE)(((int)(*pdst) + (int)c.rgbBlue)>>1); 
							*pdst++= (BYTE)(((int)(*pdst) + (int)c.rgbGreen)>>1); 
							*pdst++= (BYTE)(((int)(*pdst) + (int)c.rgbRed)>>1);	
						}
						else{
							*pdst++= c.rgbBlue; 
							*pdst++= c.rgbGreen; 
				...
				if (cx!=head.biWidth || cy!=head.biHeight){
					...
					for(yy=0;yy<desth;yy++){
						...
						for(xx=0;xx<destw;xx++){
							...
							if ((head.biClrUsed && ci!=cit) || (!head.biClrUsed && *pc!=*pct) || !bTransparent){
								// DJT, assume many pixels are fully transparent or opaque and thus avoid multiplication
								pIterSegmentation = pSegmentation + sy * head.biWidth + sx;
								getRGBColor (pIterSegmentation, cSeg.rgbRed, cSeg.rgbGreen, cSeg.rgbBlue);
								if ((eShowSegmType & IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT) == IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT){
									c.rgbBlue = (BYTE)(((int)c.rgbBlue + (int)cSeg.rgbBlue)>>1); 
									c.rgbGreen = (BYTE)(((int)c.rgbGreen + (int)cSeg.rgbGreen)>>1); 
									c.rgbRed = (BYTE)(((int)c.rgbRed + (int)cSeg.rgbRed)>>1);	
								}
								else{
									c.rgbBlue = cSeg.rgbBlue; 
									c.rgbGreen = cSeg.rgbGreen; 
									c.rgbRed = cSeg.rgbRed;	
								}
								if (a == 0) {			// Transparent, retain dest 
									pdst += 3;
								} else if (a == 255) {	// opaque, ignore dest 
					...
					for(yy=0;yy<desth;yy++,iy++){
						...
						for(xx=0;xx<destw;xx++,ix++){
							...
							if ((head.biClrUsed && ci!=cit) || (!head.biClrUsed && *pc!=*pct) || !bTransparent){
								// DJT, assume many pixels are fully transparent or opaque and thus avoid multiplication
								pIterSegmentation = pSegmentation + yy * head.biWidth + xx;
								getRGBColor (pIterSegmentation, cSeg.rgbRed, cSeg.rgbGreen, cSeg.rgbBlue);
								if ((eShowSegmType & IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT) == IGLibrary::IGFRAMEPROPERTY_SHOWSEGMENTATION_TRANSPARENT){
									c.rgbBlue = (BYTE)(((int)c.rgbBlue + (int)cSeg.rgbBlue)>>1); 
									c.rgbGreen = (BYTE)(((int)c.rgbGreen + (int)cSeg.rgbGreen)>>1); 
...
//long CxImage::DrawString(HDC hdc, long x, long y, const char* text, RGBQUAD color, const char* font, long lSize, long lWeight, BYTE bItalic, BYTE bUnderline, bool bSetAlpha)
{
	if (IsValid()){
		...
		//choose the font
		HFONT m_Font;
		LOGFONT* m_pLF = (LOGFONT*)calloc(sizeof(LOGFONT),1);
...
void CxImage::InitTextInfo( CXTEXTINFO *txt )
{
    ...
    swprintf (txt->lfont.lfFaceName, _countof (txt->lfont.lfFaceName) - 1, _T("Arial")); //use TCHAR mappings <Cesar M>
    ...
    // the text 
    swprintf (txt->text, _countof (txt->text) - 1, _T("Sample Text 01234õû")); // text use TCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}

HANDLE CxImage::convertDDBToDIB (HBITMAP bitmap, DWORD dwCompression, bool bCopyAlpha)
{
	HANDLE hDib = 0;
	if (bitmap && dwCompression == BI_RGB)
	{
		...
		if (hClientDC)
		{
			...
			if (hMemDC)
			{
				...
				if (hDib)
				{
					...
					*pbih = bih;
					// Initialise width and Height bitmap
					::GetDIBits(hMemDC, bitmap, 0, 0, NULL, (LPBITMAPINFO)pbih, DIB_RGB_COLORS);
					bih = *pbih;
					bih.biPlanes = 1;
					bih.biBitCount = 24;
					bih.biCompression = dwCompression;
					bih.biSizeImage = 0;
					*pbih = bih;
					// Initialise size of image for BitCount and Compression
					::GetDIBits(hMemDC, bitmap, 0, bih.biHeight, NULL, (LPBITMAPINFO)pbih, DIB_RGB_COLORS);
					bih = *pbih;
					...
					::GlobalUnlock(hDib);
					if ((handle = ::GlobalReAlloc (hDib, sizeof(BITMAPINFOHEADER) + bih.biSizeImage, GMEM_MOVEABLE)) != 0)
					{
						hDib = handle;
						pbih = (LPBITMAPINFOHEADER)::GlobalLock(hDib);
						if (!::GetDIBits(hMemDC, bitmap, 0, (UINT) bih.biHeight, ((LPBYTE)pbih + sizeof(BITMAPINFOHEADER)), (LPBITMAPINFO)pbih, DIB_RGB_COLORS))
						{
							::GlobalUnlock(hDib);
							GlobalFree(hDib);
							hDib = 0;
						...
						{
							// alpha channel copy
							if (bCopyAlpha && pAlpha)
							{
								int nSizeAlpha = bih.biWidth * bih.biHeight;
								if ((handle = ::GlobalReAlloc (hDib, sizeof(BITMAPINFOHEADER) + bih.biSizeImage + nSizeAlpha, GMEM_MOVEABLE)) != 0)
								{
									hDib = handle;
									BYTE *pDstAlpha = (BYTE *)::GlobalLock (hDib) + sizeof(BITMAPINFOHEADER) + bih.biSizeImage;
									memcpy_s (pDstAlpha, nSizeAlpha, pAlpha, nSizeAlpha);
								}
							}
							::GlobalUnlock(hDib);
						}
...
void CxImage::ProgressSetRange (UINT nMax, bool bLock)
{
	::SendMessageW (info.hProgress, UM_PROGRESS_SETRANGE, nMax, bLock ? 1 : 0);
}

void CxImage::ProgressSetSubRange (int nCurStartRange, int nNbSubRanges)
{
	::SendMessageW (info.hProgress, UM_PROGRESS_SETSUBRANGE, nCurStartRange, nNbSubRanges);
}

void CxImage::ProgressStepIt()
{
	::SendMessageW (info.hProgress, UM_PROGRESS_STEPIT, 0, 0);
}

void CxImage::ProgressSetMessage (wchar_t *pwMessage)
{
	::SendMessageW (info.hProgress, UM_PROGRESS_SETMESSAGE, (WPARAM)pwMessage, 0);
}
////////////////////////////////////////////////////////////////////////////////
